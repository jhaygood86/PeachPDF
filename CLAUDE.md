...
Pure .NET HTML → PDF rendering library. No external process dependency (no Puppeteer/wkhtmltopdf/gs) — HTML parsing, CSS cascade, layout, and PDF writing all run in-process. Targets .NET 8, .NET 10, and .NET 11 (currently RC).
...
- `src/PeachDrawing.Text/` — the font and text engine, published as its own NuGet package (`PeachDrawing.Text`, no dependencies) at the same version as PeachPDF, whose dependency it is (`src/Version.props` is the one place the version lives). Everything under `Internal/Fonts/` (font loading, `FontResolver`/`Typeface`/`FontFactory`, OpenType table parsing under `Internal/Fonts/OpenType/`, `Woff2Converter`, glyph outlines, `COLR`/`CPAL`, subsetting; the OpenType readers are PDFsharp-origin, so their license headers stay intact, with the MIT text beside them) and `Internal/Text/` (shaping, bidi, script itemization, hyphenation, the Unicode data tables in `Internal/Text/Resources/`). It cannot reference PeachPDF, which is what keeps the engine independent of the PDF writer, CSS and HTML layers. **Its public API is being published area by area** (`PublicApi.txt` beside the project is the reviewed snapshot of it, and `.claude/invariants/text-public-api-must-not-mirror-a-competitor.md` carries the naming rules and the register): the `PeachDrawing.Text.Unicode` namespace and font loading and matching (`FontSet`/`TypefaceFamily`/`Typeface`, in `Typefaces/`) are public, as are a typeface's metrics, glyph mapping and advances (`Typeface.Metrics`, `TryMapRune`, `GetAdvance`) shaping (`Shaping/`: `Shaper`, `ShapeSettings`, `GlyphRun`), and glyph outlines and colour glyphs (`Outlines/`: `GlyphOutline`, `ColorPalette`, the COLR paint graph, `EmbeddedBitmap`), the MATH table (`OpenType/`: `Typeface.MathData`, `MathTable`) and font subsetting for embedding (`Export/`: `TypefaceExporter`) and variable fonts (`Typeface.WithAxes`, in `Typefaces/`), and PeachPDF reaches the engine only through that surface (the engine grants `InternalsVisibleTo` only to its own test project, so `PeachPDF` and `PeachPDF.Tests` alike see the public surface and nothing else; `Typeface.Face` is the internal `LoadedTypeface` behind a public `Typeface`). Its own notices are in `src/PeachDrawing.Text/THIRD-PARTY-LICENSES.md`.
- `src/PeachDrawing.Text.Tests/` — xUnit test suite of the engine: the font parsers and tables, shaping, bidi, script itemization, hyphenation, the Unicode conformance data and the public API snapshot. It never references PeachPDF; helpers it shares with `PeachPDF.Tests` are linked from `PeachPDF.Tests/TestSupport/`. A test that needs the engine's internals belongs here (this project alone has `InternalsVisibleTo` from the engine); tests that also need PeachPDF (the PDF writer, the HTML layers) live in `PeachPDF.Tests` and are written against the public API, so a capability such a test needs and the public API lacks is added to the API rather than reached for.
- `src/PeachPDF.Tests/` — xUnit test suite, multi-targets net8.0/net10.0, plus net11.0 when the resolved SDK supports it (see the `global.json`/`NETCoreSdkVersion` note below).
...
- If the .NET SDK is missing (e.g. a fresh sandboxed/web session), install it with `apt` — `sudo apt-get install -y dotnet-sdk-10.0` (add `aspnetcore-runtime-8.0` so the net8.0 target framework can run). The `dot.net/v1/dotnet-install.sh` route is blocked in this environment (the Microsoft CDN host returns 403 through the egress proxy), so apt is the working path. `global.json` pins SDK 10.0.100 (rollForward `latestMajor`, `allowPrerelease: true`) — apt's stable feed doesn't carry a `dotnet-sdk-11.0` package while .NET 11 is still RC, so a sandboxed session installing only `dotnet-sdk-10.0` builds/tests net8.0/net10.0 fine (see the net11.0 note below for why that's not an error).
- **net11.0 is opt-in based on whichever SDK actually resolves, with no manual configuration needed either way.** `src/Directory.Build.props` only adds `net11.0` to `TargetFrameworks` when the resolved SDK is 11.0.100+ (derived from `$(MSBuildExtensionsPath)`, not `$(NETCoreSdkVersion)` — the latter isn't reliably populated this early in project evaluation across SDK versions, confirmed empirically). `global.json` stays pinned at `10.0.100` with `rollForward: "latestMajor"` and `allowPrerelease: true`: on a machine with only the .NET 10 SDK, that resolves to it and net11.0 is quietly not offered — a contributor needs zero extra setup. On a machine that *also* has the .NET 11 RC SDK installed (including CI, which installs it in `test.yml`/`publish.yml`), the same `global.json` automatically promotes to it and net11.0 joins the list — no per-contributor or per-job `global.json` edits required either way. (This needed two rounds of empirical verification to get right: an earlier belief that `rollForward: "latestMajor"` could never cross into a prerelease SDK from a different major turned out to be a false conclusion caused by a stray `global.json` accidentally left in `src/` during testing, shadowing the real one.)

## Critical: `dotnet test` invocation

**Always pass `--framework net8.0`.** The test project multi-targets net8.0 and net10.0 (plus net11.0 when the resolved SDK supports it — see the net11.0 note above); a bare `dotnet test` builds and runs the ~3000+ test suite once per resolved target framework in one invocation. Repeated bare invocations across a long session have crashed the user's Windows machine from cumulative build/test load. Single-target coverage is sufficient for regressions; only run net10.0 or net11.0 explicitly if an issue specific to that target is actually suspected.
...
Avoid writing tests against `PeachDrawing.Text.Internal.Fonts.FontFactory` (and OpenType neighbors) without care — it caches resolved fonts in `static readonly Dictionary` fields shared process-wide, and xUnit's parallel test-class execution makes new tests here a real order-dependent-flakiness risk against the rest of the suite.
...
2. **Current C# language version** — this repo doesn't pin `LangVersion` (see `src/Directory.Build.props`), so each TFM already floats to its own SDK-default latest (net8.0 → C# 12, net10.0 → C# 14, net11.0 → C# 15, etc.) — flag code that reads like an older-version holdover (e.g. missing collection expressions, primary constructors, pattern-matching improvements) where a newer construct would be a genuine clarity/correctness win, not a style-only rewrite.
3. **Current .NET APIs for the target frameworks** — prefer the modern API over an older equivalent still in the codebase out of habit. Since this repo multi-targets `net8.0;net10.0;net11.0`, if a meaningfully-better API only exists on a newer TFM, gate it with `#if NET10_0_OR_GREATER` / `#if NET11_0_OR_GREATER` as appropriate (falling back to the older-target-compatible approach in `#else`) rather than skipping it — don't downgrade the whole codebase to the lowest common denominator when a newer target can do better.
...
- `em` font-sizes are eagerly converted to points at cascade time in `CssBox`'s `FontSize` setter (`ResolveFontSizeValueComputation` in `CssBox.StyleProperties.cs`, relative to the parent's actual font size in pt), not kept symbolic — assert against the converted `pt` value, not the original `em` string.
