...
- Paint order (`CssBox.PaintImpCore`'s stacking loop) correctly orders block → float → inline-level content per CSS2.1 Appendix E *within one stacking context's own direct participants* (including a plain block wrapper whose entire content is inline, e.g. a `<div>` around nothing but a resolved inline `<object>`/image — see `CssBox.ActsAsInline`). It does NOT preserve that relative order when a float is hoisted (`DomUtils.NeedsStackingHoist`/`FlattenStackingContext`) past its immediate, non-hoisted block/inline siblings to a more distant ancestor stacking context — which happens whenever the float's immediate container doesn't itself establish a stacking context (no `z-index`/opacity/transform of its own). The float then paints relative to that distant ancestor's whole subtree instead of interleaved with its true local siblings. Fixing this needs the hoisting mechanism to preserve local relative order against non-hoisted siblings, which touches the same float/clip-ancestor hoisting logic a prior round already hardened against a double-paint regression (`StackingContextPaintRegressionTests.cs`) — not attempted here. Real-world impact is narrow (needs a float, plus a same-level block AND inline-acting sibling, inside a container that isn't itself a stacking context), but see `Acid2FeatureVerificationTests.NormalFlowBlockFloatAndInline_PaintInAppendixEOrder`'s doc comment — this is exactly Acid2's own `.eyes`/`#eyes-a`/`#eyes-b`/`#eyes-c` shape.
