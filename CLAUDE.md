...
- Stacking-context content that must escape a plain (non-stacking-context) ancestor to compete for z-order at its true enclosing stacking context — e.g. a `position:relative;z-index` or `opacity`/`transform` box nested inside a plain `position:absolute` wrapper that has no `z-index` of its own — can also escape that ancestor's `overflow:hidden` clip in the process, since `DomUtils.FlattenStackingContext`'s hoisting is driven by the paint call stack, not an accumulated/explicit clip region. This already happened for purely out-of-flow content hoisted past plain wrapper `<div>`s under the pre-existing algorithm; it now also applies to in-flow stacking-context content hoisted for the same z-order reason. Fixing it would require reworking overflow-clip tracking to be accumulated/explicit rather than tied to the paint call stack — not attempted here. See [Stacking Context](docs/html-css-support.md#stacking-context).
