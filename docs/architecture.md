...
3. **`OpenTypeDescriptor.Shape`** — the pipeline entry point — calls, in order: `GsubShaper.Shape` (GSUB), `GposPositioner.Apply` (GPOS), drops hidden-ignorable glyphs (variation selectors, unmapped default-ignorables), and, only for a run marked for right-to-left display, reverses the shaped glyph list and remaps mirrorable glyphs (e.g. parentheses) — the final step, so it never disturbs the logical-order matching GSUB/GPOS just did.
