...
Both paths build an `SvgDocument` scene graph once (cached for the box's lifetime — `CssBoxSvg.EnsureDocument`, `ImageLoadHandler.LoadSvgFromStream`). On its first paint, the scene graph is drawn into a PDF Form XObject; later paints of the same instance at the same viewport size invoke that form, including on other pages. The form cache belongs to the PDF document, so it never carries an XObject into a different output file. Position is applied when the form is invoked, while size and `PixelsPerPoint` are part of the cache key because they affect the rendered artwork. Separate `SvgDocument` instances do not share a form: inline `currentColor` and other resolved styles are already baked into each graph. A replaced SVG smaller than the backend's one-point form minimum is painted directly.
