...
- `Typeface` ([Fonts/Typeface.cs](https://github.com/jhaygood86/PeachPDF/blob/main/src/PeachDrawing.Text/Internal/Fonts/Typeface.cs)) — one resolved font face, independent of any size: it owns the `OpenTypeFontface` and the `OpenTypeDescriptor` (metrics, glyph mapping, shaping and outlines in design units). The PDF writer's `XFont` is just a `Typeface` plus an em size.
