...
A box that establishes a CSS Multi-column formatting context (`column-count`/`column-width` resolved to other than `auto`) is entered from `CssBox.PerformLayoutImp` the same way flex/table are, in place of the normal block-children loop. A column is a fragmentainer in [CSS Fragmentation Level 3](https://www.w3.org/TR/css-break-3/) terms, but only pages are modelled as fragmentainers, and this container is **monolithic** to the fragmentation driver (see [Pagination](#pagination) below) — it re-bands whole children itself rather than letting layout stop and resume inside it. Real inline-level fragmentation does now exist for pages, so wiring it into this engine is available work rather than a missing capability. `CssLayoutEngineColumns` fragments at whole-child granularity, in two phases:
