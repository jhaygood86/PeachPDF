...
`OpenTypeDescriptor.Shape` maps a source run through the font's OpenType Layout tables rather than assuming one codepoint equals one glyph. The GSUB stage handles single, multiple, alternate, ligature, contextual/chaining-context, and reverse-chaining substitutions, including default-on `ccmp`/`locl` processing and CSS-selected feature tags. Script-specific preparation supplies Arabic/Syriac-family joining forms and USE syllable formation/reordering for Devanagari, Bengali, Gujarati, and Tamil. GPOS then applies pair/class kerning, cursive attachment, mark attachment, and contextual positioning. Shaped glyphs retain their source-cluster spans and positioning deltas so measurement, visible painting, `/ToUnicode`, and `/ActualText` all describe the same result.

Line layout also runs the Unicode Bidirectional Algorithm, including explicit embeddings/overrides/isolates, weak and neutral resolution, per-line level reordering, and mirrored paired characters. Shaping runs in logical order where the script requires it; display reordering happens afterward, with the positionally aligned logical source retained for text extraction. See [Text shaping](html-css-support.md#text-shaping) for the complete feature and script boundaries.
