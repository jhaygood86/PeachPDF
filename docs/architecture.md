...
- `RGraphics.CreateTile(width, height)` creates an `XForm`/`PdfFormXObject` pair — a real PDF Form XObject — and returns a fresh `RGraphics` that paints into it. A pattern's content, or a mask's content, is rendered once into a tile this way.
...
- **Mask** renders its content into a tile, then `RGraphics.PushSoftMask`/`PopSoftMask` attach it as a real PDF `/SMask` luminosity soft mask via `PdfExtGState`, implemented in `XGraphicsPdfRenderer.PushSoftMask` ([PdfSharpCore/Drawing.Pdf/XGraphicsPdfRenderer.cs](https://github.com/jhaygood86/PeachPDF/blob/main/src/PeachPDF/PdfSharpCore/Drawing.Pdf/XGraphicsPdfRenderer.cs)) using the existing `PdfSoftMask` PDF object type.
