...
Two things stay deliberately outside the token model. **Flex, grid, table and multi-column containers are monolithic** to the driver ([§2](https://www.w3.org/TR/css-break-3/#monolithic)): each already paginates its own content, and handing it a half-laid-out subtree would break that bookkeeping — so no break escapes them. And the **retroactive movers** — the keep-with-next run pull, `break-inside: avoid`, `orphans`/`widows` — relocate content that has *already* been placed, which a forward-only record cannot express; they stay bounded corrections within a single pass, which [§4.3](https://www.w3.org/TR/css-break-3/#possible-breaks) sanctions.
