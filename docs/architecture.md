...
The driver is bounded twice over: a pass-count cap, and a check that a pass which reproduces the record it was handed lays the remainder out monolithically rather than spinning — an overflowing fragmentainer beats dropped content, which is what §4.3's own last-resort relaxation amounts to.
