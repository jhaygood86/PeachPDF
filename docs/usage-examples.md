...
Each local file's `Content-Type` is resolved from the **operating system's own MIME mechanism by default** — the shell file-association database on Windows, the Uniform Type Identifiers API on macOS/iOS, and `/etc/mime.types` on Linux — **falling back to a built-in set** when the OS provides nothing: HTML, CSS, SVG, common raster image extensions (PNG, JPEG, BMP, GIF, WebP, AVIF, TGA, PSD, HDR — this only resolves a MIME type string for the extension, independent of whether PeachPDF can actually decode that format; it decodes PNG/JPEG/BMP/GIF/WebP/AVIF, not TGA/PSD/HDR), and the TTF/OTF/WOFF/WOFF2 font formats. Anything else resolves to `application/octet-stream`.
