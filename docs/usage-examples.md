...
`serif`, `sans-serif`, `monospace`, `cursive`, `fantasy`, `math`, and `system-ui` all resolve to a real installed font, matching actual Chromium behavior per platform rather than one invented cross-platform table:
...
| `math` | Latin Modern Math, else Cambria Math | Latin Modern Math, else STIX Two Math | Noto Sans Math, else STIX Two Math | *(first installed of Latin Modern Math, STIX Two Math, DejaVu Math TeX Gyre; not asked of fontconfig)* |
