...
- **Contextual/chaining-context substitution** (GSUB lookup types 5/6, `calt` and any `font-feature-settings` tag a font routes through one) supports all three OpenType subtable formats (glyph-sequence, class-based, and coverage-based). Backtrack/input/lookahead matching for these lookups does not itself consult `GDEF` mark filtering (unlike ligature substitution above) — a font whose contextual rule depends on skipping an intervening mark inside the matched window may under-match; the common case (no such mark) is unaffected.
- **Kerning and mark positioning** (`GPOS`) apply a font's pair/class kerning (`kern`, gated by [`font-kerning`](#color--typography)) and mark-to-base/mark-to-mark diacritic attachment (`mark`/`mkmk`, always applied when the font defines it — not a stylistic opt-out).
...
- **Complex-script shaping** (Arabic joining and initial/medial/final/isolated contextual forms, Indic reordering, `GPOS` cursive attachment) is not performed — each codepoint uses its nominal glyph and mark-positioned/kerned placement regardless of script-specific joining rules.
...
SVG `<text>`/`<tspan>`/`<textPath>` shape via GSUB (ligatures always on, `font-variant-ligatures` doesn't apply in SVG yet) but do not yet request `GPOS` kerning/mark positioning.
