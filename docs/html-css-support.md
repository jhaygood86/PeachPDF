...
| `filter` | [filter](https://developer.mozilla.org/en-US/docs/Web/CSS/filter) | `opacity()`, `brightness()`, `contrast()`, and `invert()` are genuinely native, real PDF color math rather than an approximation (see below). `drop-shadow()` is a vector approximation, the same status as `box-shadow`'s own blur (see below). `blur()`, `grayscale()`, `sepia()`, `saturate()`, and `hue-rotate()` are accepted at parse time — a value like `filter: grayscale(1) opacity(0.5)` is valid, and its `opacity()` still applies — but contribute no visual effect of their own; see [Unsupported CSS Features](#unsupported-css-features) for why. `backdrop-filter` is entirely unsupported, for a different and more fundamental reason — also see that section. Per [Filter Effects 1 §3](https://www.w3.org/TR/filter-effects-1/#FilterProperty), any value other than `none` establishes a stacking context (see [Stacking Context](#stacking-context) above). |
...
`drop-shadow(<offset-x> <offset-y> <blur-radius>? <color>?)` reuses `box-shadow`'s own concentric-fill blur approximation (see [Box Shadow](#box-shadow) above), keyed off the filtered element's own border-box rectangle rather than a true per-pixel alpha silhouette of its actual painted content. A shadow behind non-rectangular content — rotated text, a PNG with transparent corners, an element clipped to an irregular `clip-path` shape — approximates that content's bounding rectangle rather than its real outline, the same rectangular-approximation caveat `box-shadow`'s own blur already carries.
...
| `text-decoration-line` | [text-decoration-line](https://developer.mozilla.org/en-US/docs/Web/CSS/text-decoration-line) | `none`, `underline`, `overline`, `line-through`, and any space-separated combination of them (e.g. `underline overline` draws both lines). A decoration declared on a block-level box decorates that box's inline content — one line per line box, as wide as that line's content — rather than the box's full width, and it reaches the inline content of in-flow block-level descendants too; floated and absolutely positioned descendants are not decorated, and the decorating box's own padding and border are skipped while a descendant inline box's are not. An atomic inline — an image, an `inline-block`, an `inline-table` — is not decorated: the line breaks around its margin box and resumes after it (which box edge bounds that gap is left to the user agent by the spec; this engine uses the margin box, so an atomic inline's own margins are undecorated too). **Decoration geometry is horizontal-only**: under a vertical `writing-mode` the line is still drawn along the physical x-axis, so it appears as a short stroke across the start of a column rather than running alongside it — a decoration on vertical text is best avoided for now |
| `text-decoration-skip-ink` | [text-decoration-skip-ink](https://developer.mozilla.org/en-US/docs/Web/CSS/text-decoration-skip-ink) | `auto`, `none`, `all`. `auto` — the initial value — interrupts an underline or overline wherever it would cross a glyph's ink, leaving a gap either side that grows with the decoration thickness (up to a cap); `none` draws the line straight through. A `line-through` is never interrupted, per spec. The line breaks once per glyph, across everything that glyph puts in the line's path — so a letter whose ink the line meets in several places, such as the two sides of an `o` or the bowl of a `g`, gets one gap spanning the whole letter rather than one per stroke, and no fragment of line is left stranded inside it. The spec leaves this choice of shape to the user agent; this matches what browsers do. Two cases skip nothing, and both fall back to an unbroken line: text drawn with a CID-keyed CFF font (an ordinary, non-CID CFF/OpenType-outline font's outlines are decoded), and a run painted by per-codepoint font fallback rather than the element's own font. `auto` leaves this to the user agent, so declining there is conformant; under `all` it is a known limitation. Skipping is also confined to a horizontal `writing-mode` — see the vertical-writing-mode note on `text-decoration-line` above — since the band it sweeps is measured along the physical x-axis |
| `text-decoration-style` | [text-decoration-style](https://developer.mozilla.org/en-US/docs/Web/CSS/text-decoration-style) | `solid`, `dashed`, `dotted`, `double`, `wavy`. `double` draws **two** strokes, each the resolved [`text-decoration-thickness`](https://developer.mozilla.org/en-US/docs/Web/CSS/text-decoration-thickness) with a gap of the same thickness between them, so the pair spans three times a single line rather than dividing that thickness between the two strokes as the equivalent [`border-style`](https://developer.mozilla.org/en-US/docs/Web/CSS/border-style) value does — a decoration's thickness describes the stroke, not a total, and dividing it would make a default double underline fainter than a solid one. The pair keeps the first stroke where a single one would sit and grows the second away from the text: downward for an underline and a line-through, upward for an overline, matching browsers. **One consequence to know:** because a double overline grows upward, it needs roughly two extra line-widths of room above the text — flush against the top of a page there is none, and the upper stroke falls outside the page and is not drawn. Give such a heading a little top margin. `wavy` strokes a curved path rather than a dash pattern — its centerline is offset from a single stroke's position the same direction `double`'s second stroke grows, by about one resolved thickness. As with `double`'s cross-reference deviation above, the exact wave shape (amplitude, wavelength) is left to the user agent by the [CSS Text Decoration Module Level 3](https://www.w3.org/TR/css-text-decor-3/#text-decoration-style-property) itself, not merely by this engine's own choice — browsers do not render an identical wave either. Decoration styles on SVG text follow the same rules as HTML **except** `double`, which SVG still paints as a single stroke |
...
The following triggers from the CSS specification are **not** supported, since the underlying properties themselves have no effect in PeachPDF (see [Unsupported CSS Features](#unsupported-css-features)): `isolation`, `will-change`, `contain`, and 3D `perspective`.
...
- When content-empty pages are skipped (see pagination), a kept page's own geometry (margins/sheet size) corrects itself to match its renumbered output position whenever that correction wouldn't change the page's own content-box width or height — the ordinary case, including a mirrored binding-gutter `:left`/`:right` pair. Only a `:first`/`:left`/`:right` rule whose margins or `size` genuinely change the content-box's own dimensions (an asymmetric margin override, or any `size` override) still resolves its geometry against the underlying page sequence rather than the renumbered output, when a content-empty gap precedes it.
...
| `margin` / `padding` (all four edges, and the longhands) | Inset the box's content from its slot in the margin area, per [css-page-3 §5.1](https://www.w3.org/TR/css-page-3/#margin-boxes) — a page margin box is a block-level box that accepts the box model. A declared `width`/`height` is the **content-box** dimension, with margin and padding additive on top of it, so `width: 200pt; padding: 0 5pt` occupies 210pt of the row and draws its content 200pt wide. Percentages resolve against the box's own **containing block** per axis, following [css-page-3 §6](https://www.w3.org/TR/css-page-3/#page-properties), which overrides CSS 2.1's uniform width rule here: `left`/`right` against that block's **width**, `top`/`bottom` against its **height**. The containing block ([§5.3.1](https://www.w3.org/TR/css-page-3/#margin-box-layout)) is the content band's width by the page margin's thickness for a top/bottom row, the margin's thickness by the content band's height for a left/right column, and the rectangle where two margins meet for a corner — so on a 40pt top margin, `padding-top: 5%` is 2pt, not 5% of the page width. A **negative margin** is honoured and grows the box past its band, which is how a footer can keep a fixed inset from the page edge on a page margin shallower than that inset; padding may not be negative and is clamped to zero. A box padded past its own extent is skipped rather than drawn inside out. `border` is **not** part of this: a margin box does not paint one, so it is not charged space either |
...
- **`filter: blur()`** — a [Gaussian blur](https://developer.mozilla.org/en-US/docs/Web/CSS/filter-function/blur) needs true per-pixel convolution, which no PDF operator expresses over already-composited vector content — and unlike `drop-shadow()` (see [Filters and Blend Modes](#filters-and-blend-modes) above), there's no offset or color to approximate a blur against. Stays a no-op
- **`filter: grayscale()`, `sepia()`, `saturate()`, `hue-rotate()`** — each of these [mixes color channels together](https://developer.mozilla.org/en-US/docs/Web/CSS/filter-function/grayscale) (every output channel is a function of more than one input channel). PDF's own per-channel color-transform construct (`ExtGState /TR`, ISO 32000-1 §8.6.5.3) is explicitly restricted to functions where each output component depends only on that same input component, with no way to compute one output channel from another channel's value, and no other PDF construct performs cross-channel color mixing over an already-composited result without rasterizing it. These four still parse successfully as part of a `filter` value — a chain like `filter: grayscale(1) opacity(0.5)` is valid, and its `opacity()` still applies — but contribute no visual effect of their own
- **`backdrop-filter`** — [any function](https://developer.mozilla.org/en-US/docs/Web/CSS/backdrop-filter) needs to read back content already painted behind the filtered element, and there is no PDF-legal mechanism to sample already-painted content — PeachPDF's paint pipeline is single-pass and forward-only, with nothing behind an element left to read
- **`text-shadow`**
