...
| `align-content` | [align-content](https://developer.mozilla.org/en-US/docs/Web/CSS/align-content) | `flex-start`, `flex-end`, `center`, `space-between`, `space-around`, `space-evenly`, `stretch`. The initial value `normal` behaves as `stretch`, growing every line equally to absorb the container's free cross space |
...
> **Migration note:** a column container's *items* previously had no break points among themselves at all: a `break-before`/`break-after` between two items of the same line did nothing, and `break-inside: avoid`/monolithic content on one item moved every item of every line in the container together, rather than just that item and what follows it. Both are now honored at item granularity, and two side-by-side lines relocate independently of each other — an item's own break value in one line never displaces the other. A document that relied on the old "every line moves together" behavior to keep two lines level after one of them stepped to a new page now sees only the line that actually needed to move do so. See [Forward compatibility](#forward-compatibility).
