...
**A cell whose `rowspan` crosses a page break is split at the break.** Where the boundary falls between two rows a spanning cell covers, the cell is fragmented like any other content ([CSS Tables Level 3](https://www.w3.org/TR/css-tables-3/#fragmentation)): its box closes level with the table's own bottom edge on the page it began on and opens again at the head of the next, its borders and background running the full depth of each fragment. The row that *ends* the span is carried onto the next page whole, exactly like any other row. The column-context values (`column`, `avoid-column`) name a fragmentation context a table does not establish; inside a [multi-column container](#multi-column-layout) they are the container's to act on.

> **Limitation:** only the spanning cell's *box* is split. Where the cell's own content is taller than the page it began on, that content is not continued on the next page — the cell keeps one box running past the boundary, as it did before. A cell whose content fits the page it starts on, which is the ordinary case, is split correctly.

> **Migration note:** a row that ends a `rowspan` used to be left wherever it fell, so a page boundary crossing it drew the row and the spanning cell cut through by the page edge. Such a row is now carried onto the next page like any other, and the spanning cell is split at the boundary rather than stretched across it — so a document with a `rowspan` at a page break gains a moved row, and possibly a page. The same correction fixes a spanning cell's `vertical-align` being applied twice, which sat its content lower in the cell than it should have been. See [Forward compatibility](#forward-compatibility).
...
> **Migration note:** a `<thead>` or `<tfoot>` used to repeat whenever the table had one. It is now subject to the two conditions above. The user-agent `break-inside: avoid` keeps the default unchanged, so an ordinary table renders exactly as before — a one-row header is a few percent of a page, nowhere near the quarter cap. Three documents do change: one whose group is **taller than a quarter of its page** now prints that group once instead of on every page, and may therefore **lose** pages; one that sets `break-inside: auto` on a `<thead>`/`<tfoot>`, which now means "do not repeat" rather than nothing; and one holding a `<thead>`/`<tfoot>` that is **not a row group at all** — most often under the responsive `table { display: block }` idiom — where the user-agent rule now applies to it as an ordinary block, so a group that used to be split by a page boundary is carried whole onto the next page instead. See [Forward compatibility](#forward-compatibility).
