...
| `writing-mode` | [writing-mode](https://developer.mozilla.org/en-US/docs/Web/CSS/writing-mode) | All five values parse, cascade, and inherit correctly, and correctly drive [logical box-model property](#logical-box-model-properties) resolution. `vertical-rl`/`vertical-lr` additionally get real vertical line flow for a block box holding plain inline content (text and simple nested inline elements, no block children): lines ("columns") stack along the block axis (right-to-left for `vertical-rl`, left-to-right for `vertical-lr`), text runs top-to-bottom within each column, and glyphs paint rotated 90°. [Flexbox](#flexbox) row/column axis selection, sizing, and page fragmentation are also writing-mode-aware. `sideways-rl`/`sideways-lr` still render as `horizontal-tb`. **Not yet supported under a vertical writing mode**: block children inside the vertical box, auto width shrinking to the content's own block-axis extent (auto width currently fills the available space, unaware of the writing mode), floats, absolute positioning, hyphenation, bidi reordering, `text-align`, `box-decoration-break: clone`, and Table/Multi-column axis interpretation |
