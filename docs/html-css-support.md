...
- [Monolithic content](#monolithic-content) — a replaced element, or a scroll container whose size is fixed or that sits outside ordinary block flow — moves whole to the next column rather than being cut by the boundary. An auto-height scroll container in ordinary block flow breaks across the column boundary like any other block.
