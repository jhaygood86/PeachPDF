...
- **Scroll containers** — any box whose `overflow` is not `visible` or `clip`, with one exception. A block box in ordinary block flow whose logical height is `auto` and whose logical `max-height` is `none` can break across pages between its lines, as it does when a browser prints it, provided everything inside it and around it can carry the break too (listed below). Its height grows with its content, so it has nothing to clip in the block axis on paper. That is not true when something else fixes its size, such as an `aspect-ratio`, or both `top` and `bottom` on an absolutely positioned box, so such a box stays monolithic. A panel of paragraphs, a table, or a `<pre>` code listing with `overflow: auto` are the common cases. An `inline-block` inside the wrapper is laid out in one piece, so if it crosses a page boundary each page shows its own slice of it. §2 only permits treating such boxes as monolithic, and never requires it. A percentage height or `max-height` against a containing block with no definite height behaves as `auto`/`none` here. In a vertical writing mode the logical height is the `width`. The box stays monolithic in these cases:
  ...
  - It holds content laid out apart from its block flow: a float, an absolutely or fixed positioned box, a page float, a multi-column, flex or grid container, or a table caption. So the `overflow: hidden` wrapper used to contain floats, such as a page layout with a floated menu beside a long column of text, stays monolithic.
  - A float comes before it in the same block formatting context, and so may sit beside it, as in the media-object pattern of a floated image followed by an `overflow: hidden` block.
