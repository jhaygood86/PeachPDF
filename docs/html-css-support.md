...
| `font-variant-ligatures` | [font-variant-ligatures](https://developer.mozilla.org/en-US/docs/Web/CSS/font-variant-ligatures) | `normal`, `none`, `common-ligatures`, and `no-common-ligatures` are parsed and actually control GSUB ligature substitution (see [Text shaping](#text-shaping)); `discretionary-ligatures`/`historical-ligatures`/`contextual` (and their `no-*` forms) parse and cascade but don't yet change rendering |
...
**Item content fragments across pages** for a `flex-direction: row` (or `row-reverse`) container with exactly one line — every item on that line, whether it is the only one or shares the line with others, has its own content (a line of text, or a nested block-level layout such as a `column-count`/`column-width` container) continue on the next page rather than being moved whole or cut when it does not fit. This composes with everything above: `RelocateLinesAcrossFragmentainers` still decides which page the line begins on, and each item's own content is then laid out for real once the line is there.

Item content is **not yet** fragmented for every other shape: `flex-direction: column`/`column-reverse` (the item's content is sequential along the same axis the line-to-line break points already use, a different unit of work), a wrapped (`flex-wrap: wrap`) container with more than one line, and grid items generally. For those, where the page boundary cuts an item, it can fall **through a line of the item's text**, and that line is then drawn on both pages — the part above the boundary at the foot of one, the part below it at the head of the next. Elsewhere a line always lands whole on one page or the other. The same happens to a line taller than the page's whole content band, wherever it appears, since there is no page that could hold it. Give such content a `break-inside: avoid` (or size it so a line cannot straddle) where a cut line would be unacceptable.
