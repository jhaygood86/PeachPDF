...
| `hr` | [hr](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/hr) | The rule *is* the box's border, painted by the same border code as every other block-level box, so every [`border-style`](https://developer.mozilla.org/en-US/docs/Web/CSS/border-style) works on it — `inset`/`outset`/`groove`/`ridge` shade their faces, `dotted`/`dashed` fit their pattern to the edge, `double` draws its two lines, and `border-style: none` paints nothing at all. `noshade` and `color` keep the rule flat rather than engraved, per the HTML Standard's own UA rule for them, and a flat rule takes its colour from [`color`](https://developer.mozilla.org/en-US/docs/Web/CSS/color) — so `<hr color="red">` is red, and a plain `<hr noshade>` is the UA's own gray. It is positioned by the same block-flow code, so it follows [CSS 2.1 §9.4.3](https://www.w3.org/TR/CSS21/visuren.html#relative-positioning) (a relatively-offset preceding sibling does not move it) and [§8.3.1](https://www.w3.org/TR/CSS21/box.html#collapsing-margins) (its margin collapses against the nearest *in-flow* predecessor, never a float). Three limitations, all in the rule's own box geometry rather than its painting. A rule with no explicit `height` is always 2pt tall, so when its two borders total less than that — which includes the 1px default — the remainder is left as content and paints as a gap between the two border edges, where [§10.6.3](https://www.w3.org/TR/CSS21/visudet.html#normal-block) gives it a used content height of 0. The same fixed height is what content following a rule is placed against, so a rule with a border thicker than about 1px is overlapped by what comes after it, and a container whose only child is a rule is 2pt tall whatever the rule's height. Separately, a percentage `width` on a rule resolves against a basis reduced by the rule's own left and right border widths, making it narrower than [§10.2](https://www.w3.org/TR/CSS21/visudet.html#the-width-property) requires. Declare a `height` on a bordered rule to get an exact one, give it a bottom margin to keep the next block clear, and prefer `auto` or a length to a percentage width. |
