...
- `position: fixed` elements and `background-attachment: fixed` layers keep positioning against the base page box on margin-only-overridden pages (they ride the page's content shift rather than re-resolving against that page's own margins). A `position: fixed` box's own **percentage** `left`/`top`, however, resolves against each page's own area (CSS2.1 §10.1 / CSS Position 3: "in paged media, the page area of each page") whenever the document mixes physical page sizes (a named/pseudo `@page` rule's own `size`) — an absolute-length offset is unaffected either way, since it doesn't depend on the basis at all.
