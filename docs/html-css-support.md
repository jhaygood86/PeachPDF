...
- **Text.** The text of a rasterized HTML element is drawn into the bitmap, and PeachPDF also places the same text over it as invisible, positioned text, so it can still be selected, searched and copied, and tagged-PDF structure still points at it. Text drawn by an inline `<svg>` or `<math>` inside the element is part of the bitmap only, and `text-decoration` lines are not part of the selectable overlay. Small text in a bitmap can be made sharper by the font's own TrueType hinting with `PdfGenerateConfig.TextHinting` (off by default; it never touches the PDF's vector text or layout), see [Sharper small text in bitmaps](usage-examples.md#sharper-small-text-in-bitmaps-texthinting).
