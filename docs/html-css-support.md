...
| `vertical-align` | [vertical-align](https://developer.mozilla.org/en-US/docs/Web/CSS/vertical-align) | `baseline`, `sub`, `super`, `top`, `middle`, `bottom`, `text-top`, `text-bottom` for any inline-level box relative to its line box, not just table cells (which use a separate, table-specific alignment algorithm — see [Tables](#tables)). Length and percentage values are not supported and fall back to `baseline` |
...
| `@supports` | Supported ([CSS Conditional Rules 3/4](https://developer.mozilla.org/en-US/docs/Web/CSS/@supports)). The `<supports-condition>` (`(property: value)`, `not (…)`, `(…) and (…)`, `(…) or (…)`, arbitrary parenthesized nesting) is evaluated once, against PeachPDF's actual implemented HTML/SVG property set — not merely whether the value parses — so a property that parses but is never rendered (e.g. `animation-name`, `transition`) correctly reports unsupported, and an SVG-only property (e.g. `fill`, `stroke`) correctly reports supported even inside an inline `<svg>` sharing the host document's stylesheet. A shorthand (`background`, `border`, `font`, `flex`, `grid`, …) is supported when the CSS-OM's own shorthand grammar expands the given value into longhands and every one of those longhands is itself supported — not merely whether the shorthand's *name* is recognized. The five CSS-wide keywords (`inherit`/`initial`/`unset`/`revert`/`revert-layer`) are always accepted for a recognized property. `transform` is checked against the specific set of transform functions PeachPDF's paint pipeline implements (translate/scale/rotate/skew variants, matrix, matrix3d); `perspective()`, for example, parses as valid CSS but is silently dropped at paint time, so it correctly reports unsupported. A property with no dedicated grammar modeled validates by delegating to PeachPDF's own CSS-OM parser for that property name, which is more accurate than a generic "does this parse" check but still hasn't been individually re-verified against layout/fragmentation/paint's actual handling for every such property — a known, narrow gap. A small number of presentation properties PeachPDF renders through hand-written dispatch rather than the generated property registry — `mask`, `marker`/`marker-start`/`marker-mid`/`marker-end`, `clip-path` on SVG elements — aren't yet covered by this oracle and report unsupported despite working; this is also a known, narrow gap, not a design limit. Any at-rule (`@font-face`/`@property`/`@page`/nested `@media`/`@layer`) may appear inside a true-condition `@supports` block and is collected normally |
