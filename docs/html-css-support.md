...
- **A positioned inline is not the containing block of a box nested in a float or inline-block inside it.** In a float, or in an inline-block holding block content, such a box is placed as though its containing block were at the page's top-left corner. In an inline-block holding only inline content, it is placed from the inline-block's own left edge rather than from the start of the positioned inline. A box placed directly inside the inline is unaffected.
...
- **An empty inline's own `font-size` does not make its line taller.** The line is sized from the text around the inline, so a larger font on an empty positioned inline leaves its zero-width box taller than the line.
