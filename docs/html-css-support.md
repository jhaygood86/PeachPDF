...
An absolutely positioned box inside **inline content**, such as a badge in a `<span>` in the middle of a sentence, stays out of the line. The text before and after it stays on the same line, and the inline around it is not split (as it would be around an in-flow block). An absolutely positioned box contributes nothing to its container's shrink-to-fit width either. When the nearest positioned ancestor is an **inline** element, such as a `<span style="position: relative">`, that inline is the containing block, as [CSS 2.1 §10.1](https://www.w3.org/TR/CSS21/visudet.html#containing-block-details) defines it. Its top and left edges are the padding edges of the inline's first line fragment, and its bottom and right edges are those of its last fragment; in `rtl` text, left and right come from the last and first fragments instead. So `top: 0; left: 0` sits at the start of a highlighted word, and `bottom: 100%` puts a label just above it.
