...
**Forced breaks before a first child.** `break-before` (and the legacy `page-break-before`) is likewise honored on a container's first in-flow child, which is the common `section > h1 { break-before: recto }` idiom. Per [§3.1](https://www.w3.org/TR/css-break-3/#break-between) the break point before a first child is the same break point as the one before its container, so it is resolved against whatever precedes the outermost container the element begins. Two consequences follow. A break declared on the very first thing in a document has nothing before it to break from, so no break is taken and no blank page is manufactured in front of it — which is [§4.4](https://www.w3.org/TR/css-break-3/#break-between)'s "no empty fragmentainer" rule. And the break moves the element itself: the containers it begins keep their own position and span the boundary, so a border or background on the container is drawn on both pages rather than moving with the heading.
