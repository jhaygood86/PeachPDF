...
| `position` | [position](https://developer.mozilla.org/en-US/docs/Web/CSS/position) | `static`, `relative`, `absolute`, `fixed` (renders ignoring page margins). `sticky` is accepted and participates in stacking/z-index like a positioned box, but its box is not currently placed in normal flow at all — it keeps its unlaid-out default location (in practice `(0, 0)`) rather than the `relative`-equivalent placement a PDF renderer with no scroll should give it |
...
| `justify-content` / `align-content` | [justify-content](https://developer.mozilla.org/en-US/docs/Web/CSS/justify-content) | Distributes leftover container space among the tracks: `end`, `center`, `space-between`, `space-around`, `space-evenly`. `start` is not supported and falls back to the initial (stretch-fill) behavior |
