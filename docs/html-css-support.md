...
- **Scroll containers** — any box whose `overflow` is not `visible` or `clip`, with one exception. A block box in ordinary block flow whose logical height is `auto` and whose logical `max-height` is `none` can break across pages between its lines, as it does when a browser prints it, provided everything inside it and around it can carry the break too (listed below). Its height grows with its content, so it has nothing to clip in the block axis on paper. A box whose block size is fixed stays monolithic: a non-auto `height` with no `max-height` for any `overflow` value, and for `auto` or `scroll` also an `aspect-ratio`, or both `top` and `bottom` on an absolutely positioned box. A box capped only by `max-height` (or, for `hidden`, an `aspect-ratio`) breaks like the auto-height case, as a browser prints it, unless its content actually overflows the cap. Then its clipped content could not be carried across the break, so it is kept in one piece and moves whole like any monolithic box. A tall `overflow: hidden` wrapper used to contain floats, such as a page layout with a floated menu beside a long column of text, a panel of paragraphs, a table, or a `<pre>` code listing with `overflow: auto` are the common cases. A float or `inline-block` inside the wrapper is laid out in one piece, as described under [`float`](#display--layout), so the wrapper can break around it. §2 only permits treating such boxes as monolithic, and never requires it. A percentage height or `max-height` against a containing block with no definite height behaves as `auto`/`none` here. In a vertical writing mode the logical height is the `width`. The box stays monolithic in these cases:
  - It is an `inline-block`, a float (including a page float such as `float: top`), or a flex or grid item, whatever its height, or it is inside one of those, a vertical writing-mode block, a multi-column container, a table caption or an `inline-table`.
  - It has `break-inside: avoid` (or `avoid-page`), or it is inside a box that does.
  - It holds content laid out apart from its block flow: an absolutely or fixed positioned box, a page float, a multi-column, flex or grid container, or a table caption.
