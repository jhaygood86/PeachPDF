...
- An `outside` marker (the default) sits beside its list item's **first** line, so in paged output it stays on the page that line is on — a list item whose own text carries on onto the next page keeps its bullet or number on the page the item starts on, and gets no second one where it resumes.
