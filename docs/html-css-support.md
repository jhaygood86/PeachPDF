...
| `font-family` | [font-family](https://developer.mozilla.org/en-US/docs/Web/CSS/font-family) | Full support. Generic families (`serif`, `sans-serif`, `monospace`, `cursive`, `fantasy`, `math`) and `system-ui` resolve to a real installed substitute matching actual Chromium behavior per platform (hardcoded specific families on Windows/macOS/Android, delegated to the OS's own fontconfig on Linux; `math` picks the first installed math-capable family of a per-platform list) — see [Fonts](usage-examples.md#fonts) for the full per-platform table. Every mapping is verified against the system's actually-installed fonts before use, falling back to the platform default font otherwise, so a requested substitute that isn't present never silently resolves to an arbitrary, unrelated font |
...
| `font-stretch` | [font-stretch](https://developer.mozilla.org/en-US/docs/Web/CSS/font-stretch) | The 9 CSS Fonts Level 3 keywords (`ultra-condensed` … `normal` … `ultra-expanded`); inherited and cascaded, and consulted for real face selection when a family has multiple registered faces at different stretch values (e.g. two `@font-face` rules with different `font-stretch` descriptors) — nearest-stretch matching per CSS Fonts Level 4 §5.2. A variable font's `wdth` axis follows the keyword's percentage (`semi-condensed` is 75%, `semi-expanded` 125%, and so on). Percentage/range values (the variable-font syntax) are not supported |
...
| `font-variant` | [font-variant](https://developer.mozilla.org/en-US/docs/Web/CSS/font-variant) | Real shorthand over `font-variant-caps`, `font-variant-ligatures`, `font-variant-numeric`, `font-variant-east-asian`, `font-variant-position`, `font-variant-emoji`, `font-variant-alternates`, and `font-feature-settings`, combinable in one declaration (e.g. `font-variant: small-caps common-ligatures oldstyle-nums;`) — setting it resets every covered longhand it doesn't mention back to its initial value, per CSS Cascading. `none` sets `font-variant-ligatures` to `none` and every other covered longhand to `normal` |
...
| `font-variant-emoji` | [font-variant-emoji](https://developer.mozilla.org/en-US/docs/Web/CSS/font-variant-emoji) | `normal`, `text`, `emoji` and `unicode` parse, cascade and select between the text (outline) and emoji (colour) form of a character that has both. Only *emoji presentation participating code points* — the characters Unicode lists in `emoji-variation-sequences.txt`, such as ❤ ☺ © ⌚ 👍 and, by that list, the digits, `#` and `*` — are affected; the property has no effect on any other character. `text`/`emoji` act as if U+FE0E/U+FE0F were appended to every participating character; `unicode` follows each character's `Emoji_Presentation` property (emoji-default → emoji, text-default → text); `normal` makes no choice, so `font-family` order decides as it always has. An explicit U+FE0E or U+FE0F in the content overrides the property. The presentation steers font selection, following the [cluster-matching order](https://www.w3.org/TR/css-fonts-4/#cluster-matching): the first `font-family` font that supports the requested form wins (a font supports it when its `cmap` format-14 table lists the variation sequence, otherwise when it is a colour font for emoji / an outline font for text); if none does, system fallback is searched for one that does; and if no font anywhere supports the form, the selector is ignored and the first font that merely covers the character is used — so an emoji-only font still draws its colour glyph for a `text` request. See [Per-character font matching](#per-character-font-matching-and-coverage-fallback) |
...
| `font-variation-settings` | [font-variation-settings](https://developer.mozilla.org/en-US/docs/Web/CSS/font-variation-settings) | `normal` or a comma-separated list of `"tag" <number>` pairs (a tag is four characters). Inherited, and reset by the `font` shorthand. Sets the named axes of a variable font (a font with an `fvar` table) to the given values, clamped to each axis's range; it wins over what `font-weight` and `font-stretch` select on the same axis. A tag the font has no axis for is ignored, and a font that is not variable is unaffected. The glyph outlines (TrueType outlines, through `gvar`), advance widths (`HVAR`, or `gvar`) and font-wide metrics (`MVAR`) follow the location, and the PDF embeds each distinct location as its own static font instance. A variable font with `CFF2` outlines is not supported, and the kerning and feature substitutions of the font's layout tables are applied as at the default design |
| `font-optical-sizing` | [font-optical-sizing](https://developer.mozilla.org/en-US/docs/Web/CSS/font-optical-sizing) | `auto` (the initial value) sets a variable font's `opsz` axis to the font size in CSS pixels; `none` leaves it at the font's default. Inherited, and reset by the `font` shorthand. A `font-variation-settings` value for `opsz` wins over `auto`. Only a font with an `opsz` axis is affected |
...
**SVG-in-OpenType** (the **`SVG `** table) is supported: each glyph's SVG document, plain or gzip-compressed, is rendered through the SVG engine as vector content and shared between occurrences as one form. The glyph is the element with the id `glyph<N>` (a document that covers a single glyph with no such element is drawn whole); the document's `var(--color0)`, `var(--color1)` and so on are the font's `CPAL` palette entries (so [`font-palette`](#fonts) applies), and `context-fill`, `context-stroke` and `currentColor` are the text colour. A font that also has bitmaps or `COLR` paints uses those first. The document comes from the font file and is untrusted: it is parsed without a DTD, an inflated document is capped at 4&nbsp;MiB, and only `data:` images resolve. A document that cannot be used draws the glyph's outline. Inside a rasterized element (filters, shadows) an SVG glyph is drawn as its outline in the text colour, and a glyph is clipped to a canvas of one em to its left, an em and a half above and two ems to its right, and half an em below.

Still unsupported (a color-emoji font using only these renders blank or monochrome): `COLR` glyphs in **CFF**-flavored fonts (the vector path decodes `glyf` outlines only). COLR&nbsp;v1 variable (`PaintVar*`) paints render at their default instance; Porter-Duff-only composite modes that PDF cannot express fall back to source-over; a radial gradient's nonzero *inner* radius and a linear gradient's `p2` rotation are approximated.
...
**Variation sequences.** A variation selector stays in the same run, and the same font, as the character before it. A font's `cmap` format 14 (Unicode Variation Sequences) table is read: a sequence the font gives its own glyph (a non-default record — e.g. a slashed zero for `0` + U+FE00, or an ideographic variant) draws that glyph, and for U+FE0E/U+FE0F a listed sequence also counts as the font supporting that presentation (see [`font-variant-emoji`](#color--typography)). No variation selector other than U+FE0E/U+FE0F influences which font is chosen.

Remaining boundary: a font offering *only* a format-12 subtable (with no format-4 subtable) is not used.
...
| `line-break` | [line-break](https://developer.mozilla.org/en-US/docs/Web/CSS/line-break) | `auto`, `loose`, `normal`, `strict` and `anywhere`; inherited. Sets how strictly characters that should not start a line are kept off it, as the [Unicode line breaking algorithm](https://www.unicode.org/reports/tr14/) is tailored for it: `strict` also keeps a small kana, an iteration mark and a prolonged sound mark off the start of a line, `normal` (which `auto` is) allows the wave dash and the katakana double hyphen there, `loose` also allows small kana, iteration marks, the middle dot and Japanese question and exclamation marks, and `anywhere` lets a line end after every character (grapheme cluster), which also makes a word's min-content width a single character. The tailoring is the same whatever the language of the text |
| `word-break` | [word-break](https://developer.mozilla.org/en-US/docs/Web/CSS/word-break) | `normal`, `break-all`, `keep-all`. Where a line may end inside a run of text is decided by the [Unicode line breaking algorithm](https://www.unicode.org/reports/tr14/): between ideographs and kana, after hyphens and spaces, and nowhere the algorithm forbids (before closing punctuation, after opening punctuation, inside numbers). `break-all` also allows a break between letters and digits, and `keep-all` forbids breaks between ideographs, Hangul and letters. Beyond the algorithm, no break is offered between `!`, `/` or `\|` and a following letter, so `!important` and `and/or` stay whole. Text in separate inline elements is analysed as if it were one run: the last word of one element and the first of the next are decided by the same algorithm, with any white space the markup collapsed between them, so an element boundary alone neither creates nor removes a break (`foo<b>bar</b>` stays whole, `<b>well-</b><i>known</i>` breaks after the hyphen) |
...
| `float` | [float](https://developer.mozilla.org/en-US/docs/Web/CSS/float) | `left`, `right`, `none`, the flow-relative `inline-start`/`inline-end` ([CSS Logical Properties §2.2](https://www.w3.org/TR/css-logical-1/#float-clear), which float to the line-left or line-right side of the containing block according to its `direction` — `ltr` puts `inline-start` on the left, `rtl` on the right — and stay the computed value rather than being rewritten to `left`/`right`), `footnote` ([css-gcpm-3](https://www.w3.org/TR/css-gcpm-3/#footnotes)), `top`, `bottom`, `top-bottom`, `snap`, `inside`, `outside` ([css-page-floats](https://www.w3.org/TR/css-page-floats-3/)) — see [Page floats](#page-floats-float-topbottomtop-bottomsnapinsideoutside) for the last six. `footnote` removes an inline-level element from normal flow entirely and routes its content to the page's - or, with [`float-reference: column`](#footnotes-float-footnote), its own column's - footnote area, the same "remove from flow" idea `position: running()` uses for margin boxes; see [Footnotes](#footnotes-float-footnote). A `left`/`right`/`inside`/`outside` float shares one inline formatting context with the surrounding inline content of the block it is in regardless of source order — placed beside that content's line boxes, with text wrapping around it, whether the float **precedes or follows** the content on the same line ([CSS 2.1 §9.5](https://www.w3.org/TR/CSS21/visuren.html#floats), [§9.5.1 rule 6](https://www.w3.org/TR/CSS21/visuren.html#float-position)). A shrink-to-fit box holding both (a float, an auto-width `position: absolute` box, an auto table column) is sized for the text and the float side by side, as are two adjacent floats — which are themselves placed side by side correctly. A box that establishes its own formatting context and takes its height from content — a float, an `overflow` other than `visible`, an `inline-block`, an absolutely/fixed-positioned box, a table cell or caption, a flex or grid item, a multi-column container — **contains** its floats: its height grows to cover any floating descendant hanging below its content ([CSS 2.1 §10.6.7](https://www.w3.org/TR/CSS21/visudet.html#root-height)), which is what makes the `overflow: hidden` containment idiom work. An ordinary block correctly does not, so a float still overhangs it. `display: flow-root`, css-display-3's dedicated way to ask for containment, is not implemented and computes as `block`. A `left`/`right`/`inside`/`outside` float whose own content is taller than fits the remaining page shows all of it: its content lays out unbroken and runs on past the page's foot, each page showing the slice that falls in it, the same way a scroll container's or an inline-block's tall content does. It is not split at its own break points, so it does not continue *beside* the text that flows around it onto the next page |
| `float-reference` | [css-page-floats](https://www.w3.org/TR/css-page-floats-3/) | `inline` (initial), `column`, `region`, `page` - which fragmentation context a float is positioned relative to. Consumed by a `float: footnote` source and by a `top`/`bottom`/`top-bottom`/`snap` page float. `column` routes a footnote into the note area of the column its reference landed in (see [Footnotes](#footnotes-float-footnote)), and pins a page float to the block-start or block-end edge of that column, reserving room for it there and only there (see [Page floats](#page-floats-float-topbottomtop-bottomsnapinsideoutside)); outside a multi-column container it falls back to the page. `page` is the page-level behavior, and `inline` (the initial value) and `region` resolve against the page too - a footnote has no inline note area to fall back to, a page float keeps the page-level default documents written for other page-float implementations expect, and PeachPDF implements no part of CSS Regions. MDN has no page for this property, so the specification is linked directly |
...
| `clear` | [clear](https://developer.mozilla.org/en-US/docs/Web/CSS/clear) | `left`, `right`, `both`, `none`, and the flow-relative `inline-start`/`inline-end` ([CSS Logical Properties §2.2](https://www.w3.org/TR/css-logical-1/#float-clear)), which clear the floats on that side of the containing block according to its `direction`, as `float: inline-start`/`inline-end` place them |
...
- [Monolithic content](#monolithic-content) — a replaced element or a scroll container — moves whole to the next column rather than being cut by the boundary. That includes an auto-height scroll container, which breaks like any other block in ordinary block flow but stays whole inside a multi-column container.
...
A **floated** child of a multi-column container is positioned within the column it appears in, and the container's automatic height grows to contain its floats. A float wider than its column overflows it rather than being narrowed. A container holding only floats lays them out in its first column. Two limitations: a float that does not fit below the content in its column stays there and overflows it, instead of moving to the next column, and column balancing does not count the space a float takes. Text placed directly in a multi-column container, with no block around it, is not divided into columns at all, so a float beside such text shortens its lines but does not affect columns.
...
- **Scroll containers** — any box whose `overflow` is not `visible` or `clip`, with one exception. A block box in ordinary block flow whose logical height is `auto` and whose logical `max-height` is `none` can break across pages between its lines, as it does when a browser prints it, provided everything inside it and around it can carry the break too (listed below). Its height grows with its content, so it has nothing to clip in the block axis on paper. A box whose block size is fixed stays monolithic: a non-auto `height` with no `max-height` for any `overflow` value, and for `auto` or `scroll` also an `aspect-ratio`, or both `top` and `bottom` on an absolutely positioned box. A box capped only by `max-height` (or, for `hidden`, an `aspect-ratio`) breaks like the auto-height case, as a browser prints it, unless its content actually overflows the cap. The space a break leaves unused at the foot of a page counts against the cap, so content that fits under it with less than that to spare counts as overflowing. Then its clipped content could not be carried across the break, so it is kept in one piece and moves whole like any monolithic box. A tall `overflow: hidden` wrapper around paragraphs, a panel, a table, or a `<pre>` code listing with `overflow: auto` are the common cases. §2 only permits treating such boxes as monolithic, and never requires it. A percentage height or `max-height` against a containing block with no definite height behaves as `auto`/`none` here. In a vertical writing mode the logical height is the `width`. The box stays monolithic in these cases:
  - It is an `inline-block`, a float (including a page float such as `float: top`), a flex or grid item, or an absolutely or fixed positioned box, whatever its height.
  - It is inside an `inline-block`, a float, an absolutely or fixed positioned box, a vertical writing-mode block, a multi-column container, a table caption or an `inline-table`. (Inside a flex or grid item, or a table cell, it can still break.)
  - It has `break-inside: avoid` (or `avoid-page`), or it is inside a box that does.
  - It holds a float, an `inline-block`, `inline-table` or image, or content laid out apart from its block flow: an absolutely or fixed positioned box, a page float, a multi-column, flex or grid container, or a table caption. The clearfix wrapper around a floated menu is therefore still kept whole.

  Such a box that is taller than a page is sliced as described below. The root element is excluded, because its `overflow` propagates to the viewport rather than making it a scroll container ([CSS Overflow Level 3 §3.3](https://www.w3.org/TR/css-overflow-3/#overflow-propagation)); a paginated document has no viewport for it to propagate to, so the common `html { overflow: hidden }` idiom does not declare a whole document unbreakable. `<body>` is excluded on the same grounds, but only while the root's own `overflow` is `visible` — once the root declares one, the body is a scroll container in its own right. (`overflow: clip` is not currently implemented and computes as `visible`, so it is excluded either way.)
...
- **`float-reference: column`** — inside a multi-column container, pins the float to the block-start or
  block-end edge of the *column* its source position falls in, rather than the page's. Only that column
  gives up the room: its neighbours keep their full height, and a column with a float at its top starts
  its content below the float while the column beside it starts at the top of the band. A
  `column`-referenced float outside any multi-column container falls back to the page.

  ```html
  <div style="column-count: 2;">
    <div style="float: top; float-reference: column; height: 60pt;">Spans only its own column.</div>
    <p>…</p>
  </div>
  ```
...
**Limitations:**
- A page float taller than the whole page's content band is placed at the top of its page and its content
  carries on across the following pages, none of it lost. Nothing is reserved for it (a strip as tall as
