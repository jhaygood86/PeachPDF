...
| `box-decoration-break` | [box-decoration-break](https://developer.mozilla.org/en-US/docs/Web/CSS/box-decoration-break) | `slice` and `clone` are both accepted and cascade normally, but the value does not change the output: painting always behaves as `slice`, the initial value — a box split across a page paints its whole border and background, which the page boundary then cuts. `clone` (a complete set of decorations repeated on each fragment) is parsed and stored, and has no effect |
