...
- **Scroll containers** — any box whose `overflow` is not `visible` or `clip`, with one exception. A block box in ordinary block flow whose logical height is `auto` and whose logical `max-height` is `none` breaks across pages like any other block, as it does when a browser prints it. Its height grows with its content, so it has nothing to clip in the block axis on paper. That is not true when something else fixes its size, such as an `aspect-ratio`, or both `top` and `bottom` on an absolutely positioned box, so such a box stays monolithic. A tall `overflow: hidden` wrapper used to contain floats, such as a page layout with a floated menu beside a long column of text, a panel of paragraphs, or a `<pre>` code listing with `overflow: auto` are the common cases. A float or `inline-block` inside the wrapper is laid out in one piece, so if it crosses a page boundary each page shows its own slice of it. §2 only permits treating such boxes as monolithic, and never requires it. A percentage height or `max-height` against a containing block with no definite height behaves as `auto`/`none` here. In a vertical writing mode the logical height is the `width`. An `inline-block`, a float (including a page float such as `float: top`), and a flex or grid item stay monolithic whatever their height, as does anything inside one of those, a vertical writing-mode block, a multi-column container, a table caption or an `inline-table`. So does a scroll container holding content that is laid out apart from its block flow: an absolutely or fixed positioned box, a page float, a multi-column, flex or grid container, or a table caption. Such a wrapper that is taller than a page is sliced as described below. The root element is excluded, because its `overflow` propagates to the viewport rather than making it a scroll container ([CSS Overflow Level 3 §3.3](https://www.w3.org/TR/css-overflow-3/#overflow-propagation)); a paginated document has no viewport for it to propagate to, so the common `html { overflow: hidden }` idiom does not declare a whole document unbreakable. `<body>` is excluded on the same grounds, but only while the root's own `overflow` is `visible` — once the root declares one, the body is a scroll container in its own right. (`overflow: clip` is not currently implemented and computes as `visible`, so it is excluded either way.)
