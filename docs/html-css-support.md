...
| `page-break-before` / `page-break-after` | [page-break-before](https://developer.mozilla.org/en-US/docs/Web/CSS/page-break-before) | The legacy aliases of `break-before`/`break-after`, with their own (smaller) value set: `auto`, `always`, `avoid`, `left`, `right`. They set the same underlying property, with `always` treated as `page`, `avoid` as `avoid`, and `left`/`right` passed through |
