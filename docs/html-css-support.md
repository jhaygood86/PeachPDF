...
Where a correction like that has a visible effect, it is called out in the relevant section below as a migration note.
...
| `align-content` | [align-content](https://developer.mozilla.org/en-US/docs/Web/CSS/align-content) | `flex-start`, `flex-end`, `center`, `space-between`, `space-around`, `space-evenly`, `stretch`. The initial value `normal` should behave as `stretch` here and currently packs the lines at cross-start instead, leaving the free cross space at cross-end — write `stretch` explicitly to grow the lines |
...
**A forced page break escapes the container.** `break-before: page` (and `left`/`right`/`recto`/`verso`) inside a multi-column container starts the next *page*: it names the page grid, and none of the container's columns is on another page. The columns it has opened so far end there, the content after the break opens the page the break names, and the container continues in its columns on that page. The directional values reserve their blank page here exactly as they do outside a container ([Directional page breaks](#directional-page-breaks)). `column-fill: balance` balances the columns *before* the break over the content that precedes it, since the break is where this fragment's flow ends.

One boundary: this holds for a forced break carried by the container's own child. A break declared further down — inside a wrapper inside the container — is lost entirely, producing neither a page break nor a column break.

> **Migration note:** `break-before: column`, `break-after: column` and `break-inside: avoid-column` previously parsed and cascaded with no layout effect at all. They now change where content lands inside a multi-column container, so a document that already carries them — a stylesheet written for a browser, most likely — gains column breaks it did not have before. Their behavior outside a multi-column container is unchanged: still nothing. See [Forward compatibility](#forward-compatibility).

> **Migration note:** a forced **page** break inside a multi-column container previously started the next column instead. It now starts the next page, so a document with a `break-before: page` — or a `page: <name>` transition, which forces one — inside such a container gains pages it did not have before. And a heading above content that starts the next column now starts it too, which the UA default `h1-h6 { page-break-after: avoid }` makes the ordinary case: content that used to fill a column to its foot may now leave the last heading's worth of space there. See [Forward compatibility](#forward-compatibility).
...
**A wrapping column container** — `flex-direction: column` (or `column-reverse`) with `flex-wrap: wrap` — stacks its lines along the inline axis instead: they sit side by side sharing one range down the page, so no page boundary falls between two of them and a break value declared at that point has nothing to act on. Where something in such a container may not be cut, every line moves to the next page together, so they stay level with one another. Its block-axis break points are the ones between the *items within* a line, which this pass does not take: only the container's first in-flow child speaks for the boundary above it, and a `break-before` on a later item of that line names a point inside the container and is left alone.

> **Migration note:** `break-after` on a flex or grid item previously had no effect at all — only `break-before` on the following line's items did. It now forces a break, so a document that already declares one gains a page break, and possibly a page, it did not have before. See [Forward compatibility](#forward-compatibility).

> **Migration note:** `break-after: avoid` and `break-before: avoid` between two flex lines or grid rows previously had no effect either — only `break-inside` was read. They now move the earlier line to join the later one, which the UA default `h1-h6 { page-break-after: avoid }` makes the ordinary case for a heading that is a flex item: a page that used to be filled to its foot may now leave that heading's worth of space there, with the heading itself on the following page. Two smaller corrections land with it: a directional value (`left`/`right`/`recto`/`verso`) on such an item now selects the page's *side*, inserting a blank page where one is needed, where before it opened the next page whichever side that was; and a line whose top already sits on a page's content top is no longer moved a further page by a forced break, so content that used to be attributed to the page after the one it printed on now agrees with `@page :left`/`:right`. See [Forward compatibility](#forward-compatibility).

> **Migration note:** the pass that moves a flex line used to read the container's lines in source order and assume that ran down the page. Under `flex-wrap: wrap-reverse` it does not: a relocated line used to leave the line physically below it behind and be drawn over the top of it, and a break value between two lines was paired with the line on the wrong side of the boundary. Both now read down the page. In a wrapping `flex-direction: column` container, a `break-before`/`break-after` between two lines used to move one line down the page on its own, out of line with the lines beside it; such a value is now inert there, and a line holding something that may not be cut moves every line with it instead of travelling alone. See [Forward compatibility](#forward-compatibility).
...
A consequence of that last point is worth stating plainly, because it differs from ordinary block flow: where the page boundary cuts an item, it can fall **through a line of the item's text**, and that line is then drawn on both pages — the part above the boundary at the foot of one, the part below it at the head of the next. Elsewhere a line always lands whole on one page or the other. The same happens to a line taller than the page's whole content band, wherever it appears, since there is no page that could hold it. Give such content a `break-inside: avoid` (or size it so a line cannot straddle) where a cut line would be unacceptable.
...
> **Limitation:** a row that *ends* a `rowspan` begun in an earlier row is not moved. Its own layout has already positioned the spanning cell, which belongs to that earlier row, so such a row is left where it falls — and where that crosses a page boundary it is drawn cut through by it. Every other row is moved. The column-context values (`column`, `avoid-column`) name a fragmentation context a table does not establish; inside a [multi-column container](#multi-column-layout) they are the container's to act on.
...
> **Limitation:** a repeating `<thead>` or `<tfoot>` is carried onto the pages a table **breaks** onto, not onto every page it spans. A page that a too-tall row merely overflows through has no break on it, so it gets no repeated header — as does the second and later page of a single-row table taller than one page. [CSS Tables Level 3](https://www.w3.org/TR/css-tables-3/#repeated-headers) repeats the group on every page the table covers.

> **Migration note:** the rows after a row taller than the page used to be placed back inside it — on the page it began on rather than the page it ends on — and drawn over its content. They now follow it, so a document containing such a row gains the pages that row really occupies and everything after it moves down. The same correction removes a spurious repeated header that was being drawn over that row. See [Forward compatibility](#forward-compatibility).
...
> **Limitation:** a repeating `<tfoot>` is not carried onto a page a row *continues* onto — it is drawn once, under the table's last row. At a break **between two rows** it does repeat on every page, as described above; it is only the mid-cell continuation that leaves it out.

> **Limitation:** a `<thead>` or `<tfoot>` is repeated whenever the table has one. [CSS Tables Level 3](https://www.w3.org/TR/css-tables-3/#repeated-headers) repeats a header or footer only where it carries `break-inside: avoid` and where doing so costs less than a quarter of the page for each, and PeachPDF applies neither condition — so a header that a browser would lay out once, in flow, repeats here, and a header taller than a quarter of the page repeats with nothing capping what it costs on each one.
...
> **Migration note:** a box that would have left fewer lines than `orphans` behind used to be relocated *after* the fact (and only when it fitted one page); the break now falls before it instead, which both closes up the interior gap that relocation left inside it and brings boxes taller than a page into scope. See [Forward compatibility](#forward-compatibility).

> **Migration note:** in a document with per-page left/right `@page` margins, neither line minimum used to apply at all. Both now do, decided in a final layout once the per-page measures have settled (and never across a change of measure) — so such a document can paginate differently, most visibly by no longer leaving a near-empty page where a paragraph's last line or two had been pushed onto one. See [Forward compatibility](#forward-compatibility).
...
> **Migration note:** under `slice`, an inline box crossing a page break could draw its closing border and background edge at the boundary as well as at its true end, so a decorated wrapping inline showed a spurious bar at the foot of every page it continued onto. `slice`'s decoration is now resolved against exactly the rectangles the box produced, so that bar is gone and the closing edge is drawn only where the box really ends. See [Forward compatibility](#forward-compatibility).

> **Migration note:** a box split at a **column** boundary used to draw a closed border on each fragment — a bottom border at the break in the leading column and a top border at the break in the trailing one. That is `clone`'s behavior; under `slice` (the initial value) those two edges are not there at all, so such a box now reads as one box cut by the column edge. A page break always rendered this correctly, because the page clip removed the border. Add `box-decoration-break: clone` to keep the previous appearance. See [Forward compatibility](#forward-compatibility).
...
> **Migration note:** a **block** whose content is split at a **column** boundary used to have its own top border and padding re-inserted at the head of every continuation column, whatever its `box-decoration-break` value — a strip of blank space with no border drawn in it, since the painter correctly treats a sliced continuation as having no top edge there. Under `slice` (the initial value) nothing is inserted at the break, so such a continuation now begins flush with the column's content top and the content below it moves up by that much. `clone` is unaffected: there each fragment does re-open with its own border and padding, and the room is reserved for them. See [Forward compatibility](#forward-compatibility).
