...
- **Text.** The text of a rasterized HTML element is drawn into the bitmap, and PeachPDF also places the same text over it as invisible, positioned text, so it can still be selected, searched and copied, and tagged-PDF structure still points at it. Text drawn by an inline `<svg>` or `<math>` inside the element is part of the bitmap only, and `text-decoration` lines are not part of the selectable overlay. Small text in a bitmap can be made sharper by the font's own hinting (TrueType instructions, or the hints of CFF outlines) with `PdfGenerateConfig.TextHinting` (off by default; it never touches the PDF's vector text or layout), see [Sharper small text in bitmaps](usage-examples.md#sharper-small-text-in-bitmaps-texthinting).
...
| `font-stretch` | [font-stretch](https://developer.mozilla.org/en-US/docs/Web/CSS/font-stretch) | The 9 keywords (`ultra-condensed` … `normal` … `ultra-expanded`) or a non-negative `<percentage>` of the normal width (`87.5%`); inherited and cascaded, and consulted for real face selection when a family has multiple registered faces at different stretch values (e.g. two `@font-face` rules with different `font-stretch` descriptors) — nearest-width matching per CSS Fonts Level 4 §5.2. A keyword stands for a percentage (`ultra-condensed` is 50%, `condensed` 75%, `semi-condensed` 87.5%, `normal` 100%, `semi-expanded` 112.5%, `expanded` 125%, `extra-expanded` 150%, `ultra-expanded` 200%), and a variable font's `wdth` axis is set to it, kept inside the range the face declares. The `font` shorthand takes only the keywords, as in the specification |
...
| At-rule | Notes |
|---------|-------|
| `@font-face` | `src` supports `url()` (remote/data-URI, with a comma-separated fallback list — each candidate is tried in order until one loads) and `local()`. The rule's own `font-weight`/`font-style`/`font-stretch` descriptors are authoritative for how that specific resource participates in matching, independent of what the font file's own internal tables say — this is what makes real multi-variant families (e.g. separate rules for weight 400 and 700) work reliably. The descriptors take **ranges** as well as single values (`font-weight: 100 900`, `font-stretch: 75% 125%`, `font-style: oblique 0deg 14deg`, and `auto`): a face covers every value inside its range in matching, so a request between two ranges is measured from the nearest end of each; the weight, width and slant of the requesting box then set a variable font's `wght`, `wdth` and `slnt` axes, kept inside the range (a `font-weight: 300 600` face is drawn at 600 for `font-weight: 900`, and text that still asks for more than the range reaches is faux-bolded); `font-variation-settings` still wins over both. A face declared `oblique` also serves upright text when its range includes 0. A descriptor that is left out, is `auto` or cannot be read leaves the face to what the file itself says, and a variable font then covers the range of its own axes. `unicode-range` is honored: multiple rules sharing a family but declaring different codepoint subsets each supply their own characters, and characters outside every declared subset fall back to the next family in the stack (see [Per-character font matching](#per-character-font-matching-and-coverage-fallback)). When two subset files share one internal font name (a common webfont pattern), each is still treated as its own resource. A variable font file needs one rule: the face is matched at the weight, width and slant each box asks for, from the axes in the file or the range the rule declares, see [`font-variation-settings`](#color--typography). See [Fonts](usage-examples.md#fonts) |
