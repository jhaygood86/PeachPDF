...
PeachPDF applies essentially all of [OpenType Layout](https://learn.microsoft.com/en-us/typography/opentype/spec/ttochap1) a font can exercise, including Arabic-family complex-script joining and Devanagari's Universal Shaping Engine (USE) syllable reordering: `GSUB` single, multiple, alternate, ligature, contextual/chaining-context, and reverse-chaining single substitution; `GPOS` kerning, cursive attachment, and mark positioning; a real [Unicode Bidirectional Algorithm (UAX #9)](https://www.unicode.org/reports/tr9/); Arabic/Syriac-family joining-form resolution and shaping; and Devanagari conjunct/reph formation with syllable-level glyph reordering. Indic script reordering beyond Devanagari (other USE-driven scripts, e.g. Bengali/Tamil) is not implemented. In practice:
...
- **Devanagari Universal Shaping Engine (USE) syllable reordering** classifies each character's Unicode `Indic_Syllabic_Category`/`Indic_Positional_Category` into a syllable structure (consonant, conjunct-forming halant sequence, dependent vowel sign, vowel modifier), applies a font's default glyph pre-processing (`locl`/`ccmp`/`nukt`/`akhn`), forms reph from a word-initial RA+virama sequence via the font's own `rphf` feature, applies the font's conjunct/half-form/subjoined-form features (`rkrf`/`abvf`/`blwf`/`half`/`pstf`/`vatu`/`cjct`), then reorders the resulting glyphs — repositioning a formed reph forward past the base consonant, and moving a pre-base dependent vowel sign (e.g. Devanagari's vowel sign I) backward to immediately follow the base — before a final pass applies the font's own presentation features (`abvs`/`blws`/`haln`/`pres`/`psts`). Only Devanagari is currently supported; other USE-driven scripts (Bengali, Tamil, and the rest of the Brahmic family) are not.
- Both Arabic-family joining and Devanagari USE shaping apply to HTML text only; SVG `<text>`/`<tspan>`/`<textPath>` does not yet resolve script, joining forms, or USE categories at all (still isolated nominal glyphs in logical order).
...
- **Only main-column blocks reflow.** Per-page width applies to block content whose containing block is the page area (the document's main column — direct block children of `<html>`/`<body>`), and (for the per-line text-wrap measure) requires the block itself to be an ordinary in-flow box that spans its containing block's full width. A table cell, a flex/grid item, a float, or an absolutely/fixed-positioned box each shares its containing block's width with siblings rather than claiming all of it, so none of these reflow — nor does content nested inside another (non-main-column) block, which adopts that ancestor's measure rather than its own page's, or a block with an explicit or percentage `width`. Named-page (`page: <name>`) left/right overrides reflow the same way an ordinary page margin override does.
