...
- **The group must be shorter than a quarter of the page's content band** — a quarter each for the header and the footer, per the spec's *"up to one quarter for header rows, and up to one quarter for footer rows"*. A taller group is laid out once instead. The room a repeated group takes is genuinely reserved on every page the table spans (see above), so without the cap a tall group is charged its own height out of every one of them — and a group taller than the band would leave no page able to make progress at all.

Both are decided once per table, against the band the table begins on, and a page a row continues onto inherits that decision rather than re-taking it. A table laid out with no page grid at all — a measurement pass, or an unpaginated container — has no band to take a quarter of, so only the `break-inside` condition applies there.

> **Limitation:** a `<tfoot>` that is *not* repeated is placed under the table's last row with no room reserved for it, since reserving room on every page is exactly what not repeating avoids. Where that last row ends flush with the foot of its page, the footer can hang a little way (bounded by roughly one row's height) into the bottom margin rather than moving to the next page with the row.

> **Migration note:** a `<thead>` or `<tfoot>` used to repeat whenever the table had one. It is now subject to the two conditions above. The user-agent `break-inside: avoid` keeps the default unchanged, so an ordinary table renders exactly as before — a one-row header is a few percent of a page, nowhere near the quarter cap. Two documents do change: one whose group is **taller than a quarter of its page** now prints that group once instead of on every page, and may therefore **lose** pages; and one that sets `break-inside: auto` on a `<thead>`/`<tfoot>`, which now means "do not repeat" rather than nothing. See [Forward compatibility](#forward-compatibility).
