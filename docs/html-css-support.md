...
Two boundaries. A forced **page** break inside a multi-column container starts the next column rather than the next page: which column the content lands in is a question the container answers, and the page break is degraded to the column break it implies. And a keep-with-next run is not pulled along by a column break, because the members of a run are *moved* and every column of a container begins at the same block-axis coordinate — there is nowhere to move them to.
