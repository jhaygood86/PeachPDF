...
- `break-after: avoid` / `break-before: avoid` (and `avoid-column`) keep a heading with the content it introduces at a column boundary, so a heading is not left at the foot of the column its content has moved out of.
