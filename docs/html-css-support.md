...
> **Migration note:** item content used to fragment only for a `flex-direction: row`/`row-reverse` container with exactly one line; every other shape translated an item's content as one already-measured piece, so a page boundary falling through its text either lost the remainder or drew the same line on both pages. Every shape's item content now fragments the same way, so a document relying on that old behavior (a multi-line or column-direction container whose overflow used to be visually absorbed by drawing a line twice, say) now sees its content actually continue onto the next page instead. See [Forward compatibility](#forward-compatibility).
