...
| `object-fit` | [object-fit](https://developer.mozilla.org/en-US/docs/Web/CSS/object-fit) | Supported on `<img>` (raster and SVG source): `fill` (default — stretch to the content box), `contain` (scale to fit, preserving aspect ratio, letterboxing the remainder), `cover` (scale to cover, preserving aspect ratio, cropping the overflow to the content box), `none` (the image's intrinsic size), and `scale-down` (the smaller of `none` and `contain`). Applied to the `<img>` replaced element only; `object-fit` on `<object>`/`<video>`/inline `<svg>` and other replaced elements is not yet honored (they render as `fill`). |
| `object-position` | [object-position](https://developer.mozilla.org/en-US/docs/Web/CSS/object-position) | Supported on `<img>`, sharing the [`background-position`](#backgrounds) grammar (keywords, lengths, percentages, and the edge-offset form). Positions the `object-fit`-sized image within the content box (and selects which part is visible when `cover`/`none` overflows). Defaults to `50% 50%` (center). |
...
| Property | MDN Reference | Notes |
|----------|--------------|-------|
| `color` | [color](https://developer.mozilla.org/en-US/docs/Web/CSS/color) | Full support including named colors, hex, and `rgb()`/`rgba()` in both the legacy comma form (`rgb(r, g, b)`, `rgba(r, g, b, a)`) and the CSS Color Level 4 space-separated form with an optional slash alpha (`rgb(r g b)`, `rgb(r g b / a)`, `rgba(r g b / a%)`) |
