...
- Both Arabic-family joining and USE shaping apply to HTML text only; SVG `<text>`/`<tspan>`/`<textPath>` does not yet resolve script, joining forms, or USE categories at all (still isolated nominal glyphs in logical order).
