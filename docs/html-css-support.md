...
| Property | MDN Reference | Notes |
|----------|--------------|-------|
| `break-before` | [break-before](https://developer.mozilla.org/en-US/docs/Web/CSS/break-before) | The full value set is accepted: `auto`, `avoid`, `avoid-page`, `page`, `left`, `right`, `recto`, `verso`, `avoid-column`, `column`, `avoid-region`, `region`. `page` forces a page break; `avoid` participates in keep-with-next (see the note below the table). The rest parse for conformance but currently have no layout effect — see the accepted-values note below. `always` and `all` are **not** accepted: they are [Level 4](https://www.w3.org/TR/css-break-4/) additions, outside the value set above. Write `break-before: page`, or use the legacy `page-break-before: always` |
| `break-after` | [break-after](https://developer.mozilla.org/en-US/docs/Web/CSS/break-after) | Same value set, same behavior, on the trailing side of the box |
| `break-inside` | [break-inside](https://developer.mozilla.org/en-US/docs/Web/CSS/break-inside) | `auto`, `avoid`, `avoid-page`, `avoid-column`, `avoid-region`. Only bare `avoid` is honored — it moves a box that would straddle a page boundary to the top of the next page; the three targeted forms parse but have no layout effect |
| `page-break-before` / `page-break-after` | [page-break-before](https://developer.mozilla.org/en-US/docs/Web/CSS/page-break-before) | The legacy aliases of `break-before`/`break-after`, with their own (smaller) value set: `auto`, `always`, `avoid`, `left`, `right`. They set the same underlying property, with `always` treated as `page`, `avoid` as `avoid`, and `left`/`right` stored unchanged (and, like their `break-*` spellings, currently inert — see the note below) |
...
| `box-decoration-break` | [box-decoration-break](https://developer.mozilla.org/en-US/docs/Web/CSS/box-decoration-break) | `slice` and `clone` are both accepted and cascade normally, but **the value does not change the output** — neither the initial `slice` nor `clone` is honored, and what gets painted at a fragmentation break is whatever the notes below describe. A box split across a **page** is `slice`-like: it paints its whole border and background, which the page boundary then cuts. An inline box split across **lines** is mixed: its borders are `slice`-like (the leading edge is drawn only on the first line, the trailing edge only on the last, matching a browser's `slice`), but its background color, gradient layers and `border-radius` corners are resolved against each line's own rectangle, so a rounded or gradient-filled inline that wraps renders closer to `clone` than to `slice`. A non-repeating background *image* is drawn on the first line only |
...
**Accepted-but-inert break values.** PeachPDF accepts the complete [CSS Fragmentation Level 3 §3](https://www.w3.org/TR/css-break-3/) value set on every break property, so a stylesheet written for a browser parses unchanged and its other declarations are unaffected. Only a subset changes the output today: `page` (forced page break), `avoid` on `break-before`/`break-after` (keep-with-next, below), and `avoid` on `break-inside`. Every other value is **completely** inert — not a weaker version of the behavior it names, but no break at all. In particular `left`, `right`, `recto` and `verso` (in both the `break-*` and `page-break-*` spellings) produce no page break whatsoever, rather than an unqualified break that merely lands on the wrong side; `column` and `avoid-column` are not honored by [Multi-column Layout](#multi-column-layout); the targeted avoidance values `avoid-page`, `avoid-column` and `avoid-region` are matched literally, so none of them behaves like bare `avoid`; and `region`/`avoid-region` are permanently inert, since PeachPDF has no CSS Regions fragmentation context for them to break into. All of these are stored and cascade normally — they simply have no effect on pagination. Use `page` (or the legacy `always`) for a forced break, and bare `avoid` for avoidance.
