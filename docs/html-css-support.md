...
| `text-decoration-skip-ink` | [text-decoration-skip-ink](https://developer.mozilla.org/en-US/docs/Web/CSS/text-decoration-skip-ink) | `auto`, `none`, `all`. `auto` — the initial value — interrupts an underline or overline wherever it would cross a glyph's ink, leaving a small gap either side that scales with the font size; `none` draws the line straight through. A `line-through` is never interrupted, per spec. Two cases skip nothing, and both fall back to an unbroken line: text drawn with a CFF/OpenType-outline font (`.otf`), whose outlines this engine does not decode, and a run painted by per-codepoint font fallback rather than the element's own font. `auto` leaves this to the user agent, so declining there is conformant; under `all` it is a known limitation. Skipping is also confined to a horizontal `writing-mode` — see the vertical-writing-mode note on `text-decoration-line` above — since the band it sweeps is measured along the physical x-axis |
