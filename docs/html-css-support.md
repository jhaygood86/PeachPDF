...
| `align-items` | [align-items](https://developer.mozilla.org/en-US/docs/Web/CSS/align-items) | `flex-start`, `flex-end`, `center`, `stretch`, `baseline`. Applies in **both** directions: a non-`stretch` alignment shrink-wraps each item to its fit-content cross size and positions it (a row item vertically, a column item horizontally), while `stretch` (the default) fills the line's cross size. `baseline` aligns items by their first text baseline and is only meaningful for row-direction flex — column-direction flex falls back to `flex-start`. One limit: `flex-wrap: wrap-reverse` swaps the cross-start and cross-end edges for the container's **lines** but not for the items *within* a line, so an item that does not fill its line's cross size is aligned against the unswapped edge — visible only where one line holds items of different cross sizes |
...
- The break need not fall between two of the container's own children. A block nested any distance below one is split at the column boundary too, and the content after the break starts the next column flush at that column's top, inside the continuing block's own fragment there. The continuing block's top border and padding are re-opened above it only under `box-decoration-break: clone`; under `slice` the box is cut at the break and nothing is inserted there — and this holds however deep the nesting goes, so a continuing block's own fragment begins where its content does. The multi-column container itself is not fragmented by its own columns, so its border and padding are never re-opened at a column boundary whatever its `box-decoration-break` value. See [Decorations at a break](#decorations-at-a-break).
...
- Where a line box of an inline straddles a page boundary, that one line's decoration slice is drawn on both pages: once in its own place, and once on the following page at a position above that page's content, where the page's clip normally hides it. The text itself is drawn once, on the page it belongs to.
...
- An `outside` marker (the default) sits beside its list item's **first** line, so in paged output it stays on the page that line is on — a list item whose own text carries on onto the next page keeps its bullet or number on the page the item starts on, and gets no second one where it resumes. Two limitations: inside a [multi-column container](#multi-column-layout) an item broken across a *column* boundary shows its marker beside the continuation in the later column rather than beside its first line; and a list item whose content is block-level (`<li><p>…</p></li>`) currently gets no marker at all — give such an item inline content, or wrap the block in the marker-bearing item's sibling, if the marker matters.
