...
| `orphans` | [orphans](https://developer.mozilla.org/en-US/docs/Web/CSS/orphans) | Enforced in plain (non-multi-column) block flow: if fewer than `orphans` lines of a paragraph-like box would precede a page boundary, the whole box is laid out again on the next page — a coarser-than-spec approximation (a spec-conformant UA pulls only the minimum lines needed across the break; PeachPDF moves the entire box, since it has no per-line fragmentation) that's skipped when the box itself is taller than one page (pushing it whole can't help there). Applies at a column boundary as well as a page boundary, now that a column is a fragmentainer — though with whole-child fragmentation a child is never split across a column, so an orphaned or widowed line cannot arise there in the first place |
...
**Forced breaks before a first child.** `break-before` (and the legacy `page-break-before`) is likewise honored on a container's first in-flow child, which is the common `section > h1 { break-before: recto }` idiom. Per [§3.1](https://www.w3.org/TR/css-break-3/#break-between) the break point before a first child is the same break point as the one before its container, so it is resolved against whatever precedes the outermost container the element begins. A break declared on the very first thing in a document has nothing before it to break from, so no break is taken and no blank page is manufactured in front of it — which is [§4.4](https://www.w3.org/TR/css-break-3/#break-between)'s "no empty fragmentainer" rule. For the same reason, a forced break that is not taken still counts as one for margin purposes: there is no break point in front of such a box for a margin to adjoin, so its margin is preserved rather than truncated.

Two limits are worth knowing, and both come from the same place — **only the element moves, not the container it begins**:

- A container with its own background, border or padding keeps its position and spans the boundary, so that chrome is drawn on *both* pages, and its copy on the page being left holds no content. Declare the break on the container instead where that matters.
- A heading chained to the content by `break-after: avoid` is **not** carried along when the content that moves is a container's first child, because that chain is followed between siblings and the heading is a sibling of the container. It is carried along normally when the heading and the moved box are siblings of each other.
