...
- **GSUB ligatures** (see [Text shaping](html-css-support.md#text-shaping)) apply to `<text>`/`<tspan>`/`<textPath>` the same way they do to HTML text, including the outlined (gradient/pattern-filled or stroked) path — but SVG has no `font-variant-ligatures` presentation attribute/style property of its own yet to turn them off; SVG text always shapes as if `font-variant-ligatures: normal` were set.
