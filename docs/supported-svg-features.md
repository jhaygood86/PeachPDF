...
| `opacity` | [opacity](https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/opacity) | Applied as a compositing alpha multiplied down the subtree at paint time (not inherited as a property — it composites instead, per spec). A group containing overlapping shapes with `opacity` on the group will double-blend where those shapes overlap, since PDF transparency groups aren't used — a known, documented limitation shared with this renderer's CSS `opacity` support |
...
| `spreadMethod` | [spreadMethod](https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/spreadMethod) | `pad`, `reflect`, `repeat` |
...
- **A true PDF transparency-group fix for `opacity`** — a cross-cutting rendering-engine change (would also affect CSS `opacity`, itself a pre-existing no-op — see [HTML & CSS Support](html-css-support.md#unsupported-css-features)), out of scope for this pass; the current per-shape alpha-blend approximation double-blends where a group's overlapping children share `opacity`.
