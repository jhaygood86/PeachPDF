...
coverage), and 8 (reverse chaining context single substitution, end-to-start), unwrapping Type 7
(Extension Substitution) transparently (see `PeachPDF.Text.GsubShaper`,
`PeachPDF.Fonts.OpenType.GsubTable`/`CoverageTable`/`ClassDefTable`); GDEF-based mark filtering of a
lookup's `lookupFlag`, consulted by ligature (Type 4) component matching, GSUB contextual/chaining
(Types 5/6/7/8) backtrack/input/lookahead matching, and GPOS mark-attachment/cursive base search alike
(see `PeachPDF.Text.GlyphSequenceFilter`/`PeachPDF.Fonts.OpenType.GdefTable`); GPOS Lookup Types 1/2
...
so a font's own contextual `rlig` rules (e.g. Arabic lam-alef) still match; Devanagari's Universal
Shaping Engine (USE) syllable reordering - Unicode `Indic_Syllabic_Category`/`Indic_Positional_Category`
driving a ported HarfBuzz category classifier/syllable grammar/two-pass glyph reorder
...
byte-for-byte against real HarfBuzz's own output for a real font, `uharfbuzz`, and against two PDF
renderers - see this feature's own recent-fixes entry); and a real UAX#9 Unicode Bidi Algorithm (see
...
- **A hyphenation split (`CssLayoutEngine.TryHyphenateWord`) does not carry a word's own
  ...
  creates** - a hyphenated Arabic-family-joining or Devanagari word loses real shaping across the split
  point. Pre-existing for `EffectiveJoiningForms` since Arabic joining landed; now also covers
  `EffectiveUseCategories`. Low practical impact today (this repo's `hyphens: auto` pattern data has no
  Arabic/Devanagari-language entries - see [`hyphens: auto` language coverage](docs/html-css-support.md#hyphens-auto-language-coverage)
  - so only an explicit, author-placed soft hyphen in Arabic/Devanagari text can reach this path at all)
  but not fixed here, since doing so means widening `TryHyphenateWord`'s own word-splitting logic to
  slice all three fields the same way `CssBox.AppendWordsFromText` already does, a change to shared
...
- **Indic script reordering is Devanagari-only.** `UseCategoryClassifier` only ports the subset of
  HarfBuzz's own category predicates a Devanagari codepoint can ever satisfy - a script needing medial
  ...
  `Consonant_Prefixed` - Devanagari's own repha is entirely font-GSUB-driven, see
  `GsubShaper.TryApplyRphf`'s own remarks) falls back to the classifier's `O` ("other", non-participating)
  category rather than its true one, so such text shapes with each codepoint's nominal glyph only, same
  as before this feature existed. Extending to another USE-driven script (Bengali, Tamil, etc.) means
  widening `UseCategoryClassifier`/`UseCategory`/`UseSyllableScanner`'s grammar, not a new mechanism.
- **`UseSyllableScanner`'s CGJ/ZWNJ handling is narrower than HarfBuzz's own.** A Combining Grapheme
  ...
  UCD-derived table for an edge case that's rare directly adjacent to real Devanagari text); and a CGJ
  appearing in the *middle* of a syllable's tail (rather than only between two already-scanned syllables)
  ends that syllable early instead of being skipped transparently mid-scan. Both degrade to two adjacent
  syllables instead of one rather than crashing or mis-rendering catastrophically.
...
- **`pref` (pre-base-reordering consonants) and the topographical `isol`/`init`/`medi`/`fina` features
  are not requested at all** - Devanagari has no codepoint that classifies as a pre-base-reordering
  consonant, and the topographical features exist for scripts that share Arabic's own joining-form
  model, which Devanagari doesn't use. Both are real HarfBuzz USE stages a future non-Devanagari USE
  script could need.
- **SVG `<text>`/`<tspan>`/`<textPath>` does not resolve script, joining forms, or USE categories at
  all** - Arabic-family and Devanagari text in SVG both still use isolated nominal glyphs (in logical,
  unreordered order for Devanagari), unlike HTML text. SVG's own bidi reordering is a separate
  implementation from HTML's, and unifying them for this wasn't in scope alongside HTML support landing.
...
`GsubMultipleAndContextualSyntheticTests`, `GsubReverseChainSyntheticTests`, `GposTableSyntheticTests`,
...
including a real cursive-attachment font); Devanagari USE behavior is covered by
`UseCategoryClassifierTests`, `UseSyllableScannerTests`, `UseReordererTests` (each testing its own pure
logic in isolation), and `DevanagariUseShapingCharacterizationTests`/`DevanagariUseCharacterizationTests`
(real-font, cross-checked glyph-for-glyph against real HarfBuzz's own output via `uharfbuzz`); the
remaining gaps above are pinned by their own doc comments in those files rather than by a dedicated
negative test per gap. See [Text shaping](docs/html-css-support.md#text-shaping).
