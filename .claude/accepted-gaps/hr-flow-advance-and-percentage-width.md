# `<hr>`: percentage width, and the currentColor bevel base (tracked, not accepted forever)
...
exists because `docs/html-css-support.md`'s `hr` row describes the first to readers, and that note and
