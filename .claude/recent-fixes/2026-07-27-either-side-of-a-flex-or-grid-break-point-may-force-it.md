...
item is the earlier sibling of the row after the last row it covers, so `BoxesEndingAbove` selects by
`RowStart + RowSpan - 1`, and it is the nearest end *below* the row being asked about rather than the
previous group's key: an item spanning rows 0–1 with a plain item at row 2 leaves row 1 with no group of
its own, and "the previous group" would then read the spanning item at a boundary inside it again.
