...
`max(column.ResumeContentTop, ContainingBlock.ClientTop)`, which re-inserted a *continuing* block's
top border and padding for `slice` as well as `clone`; corrected the same day — see
