...
| `TypefaceQuery` (`Weight`, `Width`, `IsItalic`, `MustCover`, `WidthPercent`, `ObliqueAngle`) | What a caller wants: the inputs to CSS face matching | CSS Fonts 4 section 5.2 matching inputs; `Width` is the OpenType `usWidthClass` 1 to 9 that the CSS `font-stretch` keywords map to, `WidthPercent` the `<percentage>` form of `font-stretch` (the scale of the `wdth` axis) and `ObliqueAngle` the `<angle>` of `font-style: oblique` |
...
| `AddOptions` (`FamilyName`, `Weight`, `IsItalic`, `Width`, `WeightRange`, `WidthRange`, `ObliqueRange`, `UnicodeRanges`) | The overrides of an `@font-face` rule | CSS `@font-face` descriptors (`font-weight`, `font-style`, `font-stretch`, `unicode-range`); the three `*Range` members are the "range of two values" form CSS Fonts 4 section 4.4 gives the first three |
| `AxisRange` (`Minimum`, `Maximum`, `Contains`, `Clamp`) | An inclusive range of values along one design axis | CSS Fonts 4 section 4.4 descriptor ranges ("a range of two values"); the type and its members are ours, and it is deliberately not a `Range`/`Interval` of the BCL because it is a closed interval of doubles that clamps |
...
| `Typeface.TryGetOutline(glyph, request, out outline)`, `Typeface.TryGetGridFittedAdvance`, `OutlineRequest` (`PixelsPerEm`, `GridFitting`), `GridFitting` (`None`, `Standard`, `Monochrome`), `GlyphOutline.IsGridFitted`, `GlyphOutline.GridFittedAdvance` | An outline at a size, optionally fitted to the pixel grid by the font's instructions | "Grid fitting" is the typography term for what "hinting" does (FreeType's docs, the OpenType and TrueType specifications and DirectWrite all use both); the property is `GridFitting`, not `Hinting`, because the API also says whether an outline *was* fitted. `PixelsPerEm` is the `ppem` of the OpenType specification. `None` and `Standard` are the plain words for "no fitting" and "the default mode", and a competitor's hinting enum uses the same two words; they are chosen for what they mean, and `Monochrome` (the black-and-white interpreter FreeType calls its v35 behaviour, as for a bilevel target) is the third value. The request is a value type rather than a growing argument list |
...
| `TypefaceQuery.Axes`, `Typeface.VariationKey` | Axis settings that ride with a match query (CSS `font-variation-settings`), and the canonical text of a typeface's location, for cache keys | CSS Fonts 4 (`font-variation-settings` applies after the weight, width and style have selected axis values); the key is ours |
