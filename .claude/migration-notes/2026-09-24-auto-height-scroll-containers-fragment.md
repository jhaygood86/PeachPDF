...
`overflow: hidden` panel of paragraphs no longer drops a line per page.
...
- anything with `break-inside: avoid`/`avoid-page`, or inside a box that has it;
- a wrapper holding content that is laid out apart from its block flow: a float, an absolutely or fixed
  positioned box, a page float, a multi-column, flex or grid container, or a table caption;
- a wrapper that a float comes before in the same block formatting context (the media-object pattern).

So a clearfix wrapper around floats, such as a floated menu beside a long column of text, is still
monolithic and still loses the line on each slice boundary when it is taller than a page.
