...
- **Kana, Hangul and CJK punctuation break.** Text in Hiragana, Katakana or Hangul used to be one unbreakable word (only U+4E00 to
  U+FA2D counted as Asian); it now breaks between characters as `normal` should, and `word-break: keep-all` keeps it together.
  Small kana, the prolonged sound mark, iteration marks and closing CJK punctuation stay off the start of a line.
...
- **No break between `!`, `/` or `|` and a letter**, following the deviation CSS Text 3 suggests, so `!important` and `and/or`
  stay whole even though the bare algorithm would allow a break after them.
