...
namespace PeachPDF.Html.Core.Dom
{
    ...
    internal class CssBox : CssBoxProperties, IDisposable
    {
        ...
        protected void PaintBackground(RGraphics g, RRect rect, bool isFirst)
        {
            if (rect is { Width: > 0, Height: > 0 })
            {
                ...
                if (BackgroundLinearGradient is { } gradient)
                {
                    ...
                    brush = g.GetLinearGradientBrush(p1, p2, stops, gradient.IsRepeating);
                }
                else if (BackgroundRadialGradient is { } radialGradient)
                {
                    ...
                    brush = g.GetRadialGradientBrush(center, radiusX, radiusY, radialStops, radialGradient.IsRepeating);
                }
                else if (BackgroundConicGradient is { } conicGradient)
                {
                    ...
                    brush = g.GetConicGradientBrush(conicCenter, outerRadius, conicColors, conicAngles);
                }
        ...
        // Inserts 15 intermediate samples per stop pair for non-sRGB color spaces.
            ...
            HueInterpolationMethod hueMethod)
        {
            ...
            var result = new System.Collections.Generic.List<(RColor, double)>(stops.Length * (kSamples + 1));
            result.Add(stops[0]);
        ...
        private static (RColor[] Colors, double[] AnglesRad) ApplyConicColorSpaceInterpolation(
            System.Collections.Generic.List<RColor> colors,
            ...
            HueInterpolationMethod hueMethod)
        {
            ...
            const int kSamples = 15;
            var outC = new System.Collections.Generic.List<RColor>(colors.Count * (kSamples + 1));
            var outA = new System.Collections.Generic.List<double>(angles.Count * (kSamples + 1));
        ...
        private static (RColor[] Colors, double[] AnglesRad) NormalizeConicStops(ParsedConicGradient g)
        {
            ...
            // Resolve positions: first=0, last=2π, nulls distributed
            ...
            // Apply fromAngle offset and expand hints
            ...
            // Apply non-sRGB color space interpolation before repeating expansion
            if (g.ColorSpace != GradientColorSpace.Srgb)
            {
                ...
                colors = new System.Collections.Generic.List<RColor>(csColors);
                angles = new System.Collections.Generic.List<double>(csAngles);
            }

            // For repeating gradients, tile in angular space
            ...
            double emPx = 16.0)
        {
            ...
            // Expand color hints into interpolated intermediate stops
            var result = new List<(RColor Color, double Position)>();
            int colorIdx = 0;
            for (int i = 0; i < stops.Length; i++)
            {
