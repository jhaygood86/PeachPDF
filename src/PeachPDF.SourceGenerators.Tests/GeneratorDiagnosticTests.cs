...
namespace PeachPDF.SourceGenerators.Tests
{
    ...
    public class GeneratorDiagnosticTests
    {
        ...
        {
            // No "area": an area-backed entry's CssBox member is itself generated by this same run (see
            // SymbolValidator's remarks), so PPG011 only still checks propertyPath/csharpDataType for the
            // handful of non-area-backed html entries (the logical-property scratch fields).
            var json = """
                {
                  "properties": [
                    { "name": "x", "inherited": false, "initialValue": null, "cssDataType": "cssom",
        ...
        {
            // MarginBlockStart is one of the logical-property scratch fields (CssBox.LogicalProperties.cs)
            // that stays hand-written and non-area-backed even after the storage-generator PR, so it's a
            // real symbol PPG011 can still check this entry against.
            var json = """
                {
                  "properties": [
        ...
        [Fact]
        public void PPG001_Fires_When_Html_Invalidates_Array_Item_Is_Not_A_String()
        {
            var json = """
                {
                  "properties": [
                    { "name": "border-top-width", "inherited": false, "initialValue": "medium",
                      ...
                      "html": { "propertyPath": "BorderTopWidth", "csharpDataType": "string", "area": "BorderArea",
                        "invalidates": ["InvalidateBorderTopWidth", 5] } }
                  ]
                }
                """;
        ...
        [Fact]
        public void PPG001_Fires_When_Html_Invalidates_Is_Neither_A_String_Nor_An_Array()
        {
            var json = """
                {
                  "properties": [
        ...
        [Fact]
        public void ValueComputation_UnrecognizedTag_FailsGenerationLoudly()
        {
            // html.valueComputation names a closed, generator-known strategy (StylePropertiesEmitter's
            // own C#, not JSON-driven) - an unrecognized tag is a mistake in the JSON entry (a typo, or a
            // future computation nobody implemented the emitter branch for yet), the same failure shape
            // KeywordOrValue_UnrecognizedValueType_FailsGenerationLoudly exercises for valueType.
            var json = """
                {
                  "properties": [
            ...
            Assert.Contains("CS8785", IdsOf(result));
            Assert.Contains(result.Diagnostics, d => d.GetMessage().Contains("valueComputation \"banana\""));
        }
    }
}
