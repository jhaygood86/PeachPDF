...
namespace PeachPDF.Tests.Html.Core.Fragmentation
{
    ...
    public class MonolithicContentTests
    {
        ...
        // Placed by its offsets, a positioned box takes no part in placing the in-flow content after it, so a
        ...
        // positioned box, a float, a page float, an atomic inline, a multi-column/flex/grid container or a
        // table caption inside it keeps it monolithic, and so does a multi-column container above it. A
        // block-level float breaks between its lines and its break loses the text beside it, and a tall atomic
        // inline lost the tail of its slice. Table parts, columns included, carry a break. display:none
        ...
        [InlineData("<div id='t' style='overflow:hidden'><div style='float:left'><div style='columns:2'>f</div></div></div>", true)]
        ...
        [InlineData("<div id='t' style='overflow:hidden'>a <span style='display:inline-block'>b</span></div>", true)]
        [InlineData("<div id='t' style='overflow:hidden'>a <table style='display:inline-table'><tr><td>b</td></tr></table></div>", true)]
        [InlineData("<div id='t' style='overflow:hidden'>a <img style='width:10pt;height:10pt'> b</div>", true)]
        ...
        // The subtree answer is kept for the layout generation that computed it, and asked again in a new
        // one, in case the tree changed.
        [Fact]
        public async Task SubtreeAnswer_IsKeptForItsGenerationOnly()
        {
            var (root, container) = await LayoutHarness.LayoutAsync(LayoutHarness.Wrap(
                "<div id='t' style='overflow:hidden'><p>a</p></div>"));
            var box = LayoutHarness.FindById(root, "t")!;
            var generation = container.LayoutGeneration;

            // A stale answer is recomputed.
            box.DescendantsCarryABreak = (generation - 1, false);
            Assert.False(MonolithicContent.IsMonolithic(box));
            Assert.Equal((generation, true), box.DescendantsCarryABreak);
        ...
        public async Task AbsolutePercentage_CapsTheBlockSizeAgainstTheBaseLayoutUses(string css, bool expected)
        {
            ...
            // Asked of the block-size test itself: an absolutely positioned scroll container is monolithic
            // whatever its size (AutoHeightScrollContainer_StaysMonolithicOutsideBlockFlow).
            Assert.Equal(expected, MonolithicContent.HasConstrainedBlockSize(LayoutHarness.FindById(root, "t")!));
        }
