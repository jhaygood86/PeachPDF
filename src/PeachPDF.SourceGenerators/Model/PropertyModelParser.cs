...
namespace PeachPDF.SourceGenerators.Model
{
    ...
    internal static class PropertyModelParser
    {
        ...
        private static void ValidateCrossEntry(List<PropertyEntry> entries, List<ModelDiagnostic> diagnostics)
        {
            ...
            // Two entries sharing an html.propertyPath (a legacy alias pair, e.g. page-break-after/
            // break-after) must also share the same html.area: AreasEmitter/StylePropertiesEmitter both
            // dedupe by propertyPath before emitting, so a propertyPath used under two different areas
            // would silently bind the generated CssBox property to only the first-seen area, and
            // SymbolValidator can't catch it either (PPG010/011 skip every area-backed entry, since that
            // CssBox member is generated by this same run). Catching the mistake here, at authoring time,
            // is the only place it can still be caught.
            var areaByPath = new Dictionary<string, (string Area, PropertyEntry Entry)>(StringComparer.Ordinal);
            foreach (var entry in entries)
            {
                var path = entry.Html?.PropertyPath;
                var area = entry.Html?.Area;
                if (path is null || area is null) continue;

                if (areaByPath.TryGetValue(path, out var seen))
                {
                    if (seen.Area != area)
                    {
                        diagnostics.Add(new ModelDiagnostic(DiagnosticCode.PropertyPathAreaConflict,
                            $"\"{entry.Name}\".html.propertyPath \"{path}\" declares area \"{area}\", but " +
                            $"\"{seen.Entry.Name}\" already declared the same propertyPath under area \"{seen.Area}\" " +
                            "(the generated CssBox property/ComputedStyleAreas field would silently bind to only one).",
                            entry.Line, entry.Column));
                    }
                    continue;
                }

                areaByPath[path] = (area, entry);
            }
        }
