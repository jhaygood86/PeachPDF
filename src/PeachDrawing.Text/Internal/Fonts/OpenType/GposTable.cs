...
// via Types 4/6), and contextual positioning - see `PeachDrawing.Text.Internal.Text.GposPositioner`.
//
...
// `PeachDrawing.Text.Internal.Text.GposPositioner`'s `TryApplyCursivePair`/`ApplyMarkToBaseAt`/`ApplyMarkToLigatureAt`),
...
namespace PeachDrawing.Text.Internal.Fonts.OpenType
{
    ...
    /// filter, see <see cref="PeachDrawing.Text.Internal.Text.GlyphSequenceFilter"/>); don't conflate the two.
    /// </summary>
    ...
    /// <see cref="PeachDrawing.Text.Internal.Text.GsubShaper"/>'s GSUB equivalent, except <see cref="LookupListIndex"/>
