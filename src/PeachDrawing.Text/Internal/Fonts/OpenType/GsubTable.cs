...
// filtering is honored via `GdefTable` (see `PeachDrawing.Text.Internal.Text.GlyphSequenceFilter`), and feature
...
namespace PeachDrawing.Text.Internal.Fonts.OpenType
{
    ...
    internal sealed class GsubTable
    {
        ...
        private readonly GsubTable? _root;
        ...
        private readonly ConcurrentDictionary<string, GsubTable> _views = new();
        ...
        /// </summary>
        public GsubTable AtLocation(Variations.VariationCoordinates location)
        {
            var root = _root ?? this;
            ...
            return root._views.GetOrAdd(location.Key, _ => new GsubTable(root, substitutions));
        }
        ...
        private Variations.FeatureVariationsTable? LoadFeatureVariations()
        {
            lock (_face.SyncRoot)
            {
                ...
                {
                    if (_face.TableDictionary.TryGetValue(TableTagNames.GSUB, out var entry))
                    {
        ...
        private GsubTable(GsubTable root, IReadOnlyDictionary<int, int[]> substitutions)
        {
            _root = root;
            ...
            _ligatureLookupCache = root._ligatureLookupCache;
            _singleSubstitutionLookupCache = root._singleSubstitutionLookupCache;
            _alternateSubstitutionLookupCache = root._alternateSubstitutionLookupCache;
            _multipleSubstitutionLookupCache = root._multipleSubstitutionLookupCache;
            _contextualLookupCache = root._contextualLookupCache;
            _chainingContextLookupCache = root._chainingContextLookupCache;
            _reverseChainSingleSubstLookupCache = root._reverseChainSingleSubstLookupCache;
        ...
        /// generically (see <see cref="PeachDrawing.Text.Internal.Text.GsubShaper"/>) don't need their own unwrap logic.
