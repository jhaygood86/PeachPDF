...
**Status:** pre-1.0. The engine is complete and in production use inside PeachPDF, but its public API is still being
designed, so this package currently contains the engine without a stable public surface. It is versioned in lockstep
with PeachPDF (the same number for every release) and is a dependency of it.
